...
function _applySSRSettings(pass) {
  ...
  // Extra quality knobs — applied whenever the underlying SSRPass build
  // exposes them; harmless no-ops otherwise (kept for forward-compat).
  if ('maxSteps' in pass || 'steps' in pass) pass[('maxSteps' in pass) ? 'maxSteps' : 'steps'] = +ssrSettings.steps || 32;
  if ('fadeDistance' in pass) pass.fadeDistance = +ssrSettings.fadeDistance || 0;
  if ('fresnel' in pass) pass.fresnel = +ssrSettings.fresnel;
  if ('minGlossiness' in pass) pass.minGlossiness = +ssrSettings.minGlossiness || 0;
}
...
// Returns the shared SSR pass whenever the render-preview lamp is active.
// There is intentionally no user-facing activation toggle: SSR is part of
// the render pipeline and its numeric/quality controls live in Render.
