...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const ICON = {
    ...
    presets:  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3h12a1 1 0 0 1 1 1v17l-7-4-7 4V4a1 1 0 0 1 1-1Z"/></svg>`,
  };
  ...
  // ── Object Presets — a "Presets" group inside the Objects panel tree,
  // right alongside the real scene objects. Long-press-drag any object row
  // onto it (same reparent gesture used elsewhere in this list) to save that
  // object/model — mesh, materials, textures AND skeleton/bones — as a
  // reusable preset. Click a saved preset to drop a fresh copy into the
  // scene. Stored in localStorage: a personal library, not part of a
  // specific project file. ───────────────────────────────────────────────────
  const OBJ_PRESETS_SENTINEL = '__objPresets__';
  ...
  // off-screen, then discards the clone.
  ...
  async function _saveObjectPreset(obj) {
    ...
    expandedGroups.add(OBJ_PRESETS_SENTINEL);
    renderObjectsList();
  }
  ...
  function buildPresetsSection() {
    const presets = _loadObjectPresets();
    const isExp = expandedGroups.has(OBJ_PRESETS_SENTINEL);
    let html = `<div class="objectItem objPresetsHeader" data-uuid="${OBJ_PRESETS_SENTINEL}" data-depth="0" style="padding-left:8px">
      <button class="groupToggleBtn" data-group-uuid="${OBJ_PRESETS_SENTINEL}" type="button"><svg class="groupArrow${isExp ? ' expanded' : ''}" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg></button>
      ${ICON.presets}
      <span class="objectName">Presets</span>
      <span class="objectChildCount">${presets.length}</span>
    </div>`;
    if (isExp) {
      if (!presets.length) {
        html += `<div class="objPresetHint" style="padding-left:30px">Segure e arraste um objeto até aqui para salvar.</div>`;
      } else {
        presets.forEach((p, i) => {
          html += `<div class="objPresetRow" data-obj-preset-index="${i}" style="padding-left:30px" title="Carregar &quot;${p.name}&quot; na cena">
            ${p.thumb ? `<img class="objPresetThumb" src="${p.thumb}" alt="">` : `<span class="objPresetThumb objPresetThumbFallback">${ICON.mesh}</span>`}
            <span class="objectName">${p.name}</span>
            <button class="objPresetDelete" type="button" data-obj-preset-delete="${i}" title="Remover preset">×</button>
          </div>`;
        });
      }
    }
  ...
  function renderObjectsList() {
    ...
    const presetsHTML = buildPresetsSection();
    if (top.length === 0) {
      objectsList.innerHTML = presetsHTML + '<div class="objectsEmpty">Cena vazia.<br>Adicione objetos pelo painel.</div>';
      attachListListeners();
      return;
    }
    let html = '';
    for (const obj of top) html += buildObjectRow(obj, 0);
    objectsList.innerHTML = presetsHTML + html;
    attachListListeners();
  }
  ...
  function attachListListeners() {
    ...
    objectsList.querySelectorAll('.objectItem').forEach((el) => {
      el.addEventListener('click', () => {
        ...
        // The Presets header isn't a real scene object — clicking it (outside
        // the arrow button, already handled above) just expands/collapses it.
        if (uuid === OBJ_PRESETS_SENTINEL) {
          expandedGroups.has(uuid) ? expandedGroups.delete(uuid) : expandedGroups.add(uuid);
          renderObjectsList();
          return;
        }
    ...
    // ── Object Presets rows — click to load, "×" to remove ──────────────────
    objectsList.querySelectorAll('.objPresetRow').forEach((el) => {
      el.addEventListener('click', (e) => {
        if (e.target.closest('[data-obj-preset-delete]')) return; // handled below
        _loadObjectPreset(Number(el.dataset.objPresetIndex));
      });
    });
    objectsList.querySelectorAll('[data-obj-preset-delete]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        _deleteObjectPreset(Number(btn.dataset.objPresetDelete));
      });
    });
  }
