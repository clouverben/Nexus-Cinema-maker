...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Pós-proc. tab now (it was never a per-object property; the shared pass
  // always covered the whole scene — see reflections.js). ─────────────────────
  const reflSSRRow           = document.getElementById('reflSSRRow');
  const reflSSRBody          = document.getElementById('reflSSRBody');
  ...
  function syncReflectionsUI() {
    if (!reflSSRRow) return;
    reflSSRRow.classList.toggle('active', !!ssrSettings.enabled);
    reflSSRBody?.classList.toggle('hidden', !ssrSettings.enabled);
  ...
  reflSSRRow?.addEventListener('click', () => {
    ssrSettings.enabled = !ssrSettings.enabled;
    reflSSRRow.classList.toggle('active', ssrSettings.enabled);
    reflSSRBody?.classList.toggle('hidden', !ssrSettings.enabled);
    markSceneDirty();
