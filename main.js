...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // own "Presets" category with a drop zone. Long-press-drag any row from
  // this Objects list onto that drop zone (same reparent gesture used
  // elsewhere in this list — see the pointer handlers near the bottom of
  // this file) to save that object/model — mesh, materials, textures,
  // skeleton/bones AND any child lights — as a reusable preset with a
  // rendered thumbnail. Click a saved preset (in the Add panel) to drop a
  // fresh copy into the scene. Stored in localStorage: a personal library,
  // not part of a specific project file. ─────────────────────────────────────
  const OBJ_PRESETS_KEY   = 'ncm_object_presets_v1';
  let   _presetPreviewRig = null; // lazily-built offscreen render rig for thumbnails

  function _loadObjectPresets() {
    try { return JSON.parse(localStorage.getItem(OBJ_PRESETS_KEY) || '[]'); }
    catch { return []; }
  }
  function _saveObjectPresetsList(list) {
    try { localStorage.setItem(OBJ_PRESETS_KEY, JSON.stringify(list)); }
    catch (e) { console.warn('[objects] Não foi possível salvar os presets:', e); }
  }

  async function _ensurePresetPreviewRig() {
    if (_presetPreviewRig) return _presetPreviewRig;
    ...
    _presetPreviewRig = { THREE, renderer, scene, camera };
    return _presetPreviewRig;
  }

  // Renders a small isolated thumbnail of `obj` — clones it (cheap: THREE's
  // clone() shares geometry/material, only duplicates the transform/hierarchy,
  // and properly re-binds skinned-mesh skeletons), frames it, renders once
  // off-screen, then discards the clone. Any light inside `obj` is part of
  // the clone too, so it lights the thumbnail exactly like it'll light the
  // real scene once loaded.
  async function _renderObjectPresetThumbnail(obj) {
    const { THREE, renderer, scene, camera } = await _ensurePresetPreviewRig();
  ...
  async function _saveObjectPreset(obj) {
    ...
    if (!modelData) { console.warn('[objects] Falha ao salvar preset de', obj.name); return; }
    if (modelData._oversized) {
      console.warn('[objects] Modelo grande demais pra virar preset:', obj.name);
      return;
    }
    let thumb = '';
    try { thumb = await _renderObjectPresetThumbnail(obj); } catch (e) { console.warn('[objects] thumbnail falhou:', e); }

    const presets = _loadObjectPresets();
    presets.push({
      name: obj.name || `Preset ${presets.length + 1}`,
      thumb,
      modelData,
    });
    _saveObjectPresetsList(presets);
    _renderAddPanelPresetsGrid();
  }

  async function _loadObjectPreset(index) {
    const presets = _loadObjectPresets();
  ...
  function _deleteObjectPreset(index) {
    const presets = _loadObjectPresets();
  ...
  // synthetic one.
  function _renderAddPanelPresetsGrid() {
    ...
    grid.innerHTML = presets.map((p, i) => `
      <div class="matPresetCard" data-add-preset-index="${i}" title="Carregar &quot;${p.name}&quot; na cena">
        ...
        <button class="matPresetDelete" type="button" data-add-preset-delete="${i}" title="Remover preset">×</button>
      </div>
    `).join('');
  }
  ...
  function buildObjectRow(obj, depth) {
    ...
    let html = `<div class="objectItem${isSel ? ' selected' : ''}${isActive ? ' active' : ''}" data-uuid="${obj.uuid}" data-depth="${depth}" style="padding-left:${pad}px">
  ...
  function attachListListeners() {
    ...
    objectsList.querySelectorAll('.objectItem').forEach((el) => {
      el.addEventListener('click', () => {
        // A long-press-drag reparent just completed on this pointer sequence —
        // swallow the trailing click so it doesn't also toggle selection.
  ...
  // Hold an object row for 1.5s, then drag onto another row to make it a
  // child of that object. (Replaces the old dragstart/dragover/drop wiring —
  // rows were never marked draggable="true" so it never fired, and native
  // HTML5 DnD doesn't work with touch input in a WebView anyway.)
  const LONG_PRESS_MS       = 1500;
  const LONG_PRESS_MOVE_TOL = 10;   // px of finger movement that cancels the hold

  let _lpTimer      = null;
  let _lpPointerId  = null;
  let _lpStartX     = 0, _lpStartY = 0;
  let _lpSrcUUID    = null;
  let _dragActive   = false;
  let _dragOverUUID = null;
  let _dragOverAddPresets = false;
  let _suppressNextClick = false;

  function _clearDragVisuals() {
    objectsList.querySelectorAll('.pressing, .drag-src, .drag-over').forEach((el) => {
      el.classList.remove('pressing', 'drag-src', 'drag-over');
    });
    document.getElementById('addPresetsDropZone')?.classList.remove('dragOver');
  }
  function _resetDragState() {
    if (_lpTimer) { clearTimeout(_lpTimer); _lpTimer = null; }
    _lpPointerId = null; _lpSrcUUID = null; _dragActive = false; _dragOverUUID = null;
    _dragOverAddPresets = false;
    _clearDragVisuals();
  }

  objectsList.addEventListener('pointerdown', (e) => {
    if (_lpPointerId !== null) return;   // a press/drag is already in progress
    if (e.pointerType === 'mouse' && e.button !== 0) return;
    const row = e.target.closest('.objectItem');
    if (!row || e.target.closest('.groupToggleBtn')) return;
    _lpPointerId = e.pointerId;
    _lpStartX = e.clientX; _lpStartY = e.clientY;
    _lpSrcUUID = row.dataset.uuid;
    row.classList.add('pressing');
    try { objectsList.setPointerCapture(e.pointerId); } catch {}
    _lpTimer = setTimeout(() => {
      _lpTimer = null;
      if (_lpPointerId === null) return;
      _dragActive = true;
  ...
  objectsList.addEventListener('pointermove', (e) => {
    if (e.pointerId !== _lpPointerId) return;
    if (!_dragActive) {
      const dx = e.clientX - _lpStartX, dy = e.clientY - _lpStartY;
      if (dx * dx + dy * dy > LONG_PRESS_MOVE_TOL * LONG_PRESS_MOVE_TOL) _resetDragState();
      return;
    }
    e.preventDefault();   // holding + dragging shouldn't also scroll the list
    const el   = document.elementFromPoint(e.clientX, e.clientY);
    const row  = el?.closest?.('.objectItem');
    const uuid = (row && row.dataset.uuid !== _lpSrcUUID) ? row.dataset.uuid : null;
    if (uuid !== _dragOverUUID) {
      objectsList.querySelectorAll('.drag-over').forEach((n) => n.classList.remove('drag-over'));
      _dragOverUUID = uuid;
      if (uuid && row) row.classList.add('drag-over');
    }

    // The drop zone lives in the ADD panel (left side) — a different part of
    // the DOM entirely — so this is a plain point-in-rect check, not scoped
    // to objectsList like the row check above.
    const presetZoneEl = document.getElementById('addPresetsDropZone');
    const overAddPresets = !!(presetZoneEl && el && presetZoneEl.contains(el));
    if (overAddPresets !== _dragOverAddPresets) {
      _dragOverAddPresets = overAddPresets;
      presetZoneEl?.classList.toggle('dragOver', overAddPresets);
    }
  }, { passive: false });

  objectsList.addEventListener('pointerup', (e) => {
    if (e.pointerId !== _lpPointerId) return;
    const wasDragging = _dragActive;
    const srcUUID = _lpSrcUUID, tgtUUID = _dragOverUUID, droppedOnAddPresets = _dragOverAddPresets;
    _resetDragState();
    if (!wasDragging) return;   // quick tap — let the normal click handler run
    e.preventDefault();
    _suppressNextClick = true;
    setTimeout(() => { _suppressNextClick = false; }, 400);   // safety net
    if (!srcUUID) return;
    const srcObj = app.scene.getObjectByProperty('uuid', srcUUID);
    if (!srcObj) return;
    if (droppedOnAddPresets) {
      _saveObjectPreset(srcObj);
      return;
    }
    if (!tgtUUID || srcUUID === tgtUUID) return;
  ...
  objectsList.addEventListener('pointercancel', (e) => {
    if (e.pointerId !== _lpPointerId) return;
    _resetDragState();
  });
  ...
  document.getElementById('addPresetsGrid')?.addEventListener('click', (e) => {
    ...
    if (delBtn) {
      e.stopPropagation();
      _deleteObjectPreset(Number(delBtn.dataset.addPresetDelete));
      return;
    }
    const card = e.target.closest('[data-add-preset-index]');
    if (card) _loadObjectPreset(Number(card.dataset.addPresetIndex));
  });
  _renderAddPanelPresetsGrid();
