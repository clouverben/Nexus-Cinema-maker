...
import { ssrSettings, syncSSRSettings, setSSRActive } from './reflections.js';
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  function syncPanelExtension() {
    ...
    // Material extension now contains only the viewer; reflections live in Render.
  }
  ...
  // ── Render-panel reflections ─────────────────────────────────────────────
  // Reflection is global now. No CubeCamera, no per-object enable toggles.
  const reflOpacity       = document.getElementById('renderReflOpacity');
  const reflMaxDistance   = document.getElementById('renderReflMaxDistance');
  const reflThickness     = document.getElementById('renderReflThickness');
  const reflResolution    = document.getElementById('renderReflResolutionScale');
  const reflBlur          = document.getElementById('renderReflBlur');

  function syncRenderReflectionUI() {
    if (reflOpacity) reflOpacity.value = ssrSettings.opacity;
    if (reflMaxDistance) reflMaxDistance.value = ssrSettings.maxDistance;
    if (reflThickness) reflThickness.value = ssrSettings.thickness;
    if (reflResolution) reflResolution.value = ssrSettings.resolutionScale;
    if (reflBlur) reflBlur.value = ssrSettings.blur ? 1 : 0;
  }
  function bindRenderReflNumber(input, key) {
  ...
  bindRenderReflNumber(reflOpacity, 'opacity');
  bindRenderReflNumber(reflMaxDistance, 'maxDistance');
  bindRenderReflNumber(reflThickness, 'thickness');
  bindRenderReflNumber(reflResolution, 'resolutionScale');
  reflBlur?.addEventListener('input', () => {
    ssrSettings.blur = Number(reflBlur.value) >= 1;
    syncSSRSettings();
    markSceneDirty();
  });
  syncRenderReflectionUI();
  ...
  renderLightToggleBtn?.addEventListener('click', () => {
    ...
    window.ncmShowRenderReveal?.(next);
  });
