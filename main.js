...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  let bonePanelBeforeMaterial = false;
  const materialLabSection = document.getElementById('materialLabSection');
  const objectsSectionEl = document.getElementById('objectsSection');
  const rightPanelEl = document.getElementById('rightPanel');
  const boneRevealBtnEl = document.getElementById('boneRevealBtn');

  function syncMaterialLabDock(active) {
    if (!materialLabSection || !objectsSectionEl) return;
    if (active) {
      bonePanelBeforeMaterial = !!bonePanelVisible;
      if (bonePanelVisible) closeBonePanel();
      materialLabSection.classList.remove('hidden');
      objectsSectionEl.classList.add('split');
      rightPanelEl?.classList.add('material-context');
      document.body.classList.add('material-panel-active');
      if (boneRevealBtnEl) boneRevealBtnEl.classList.add('hidden');
    } else {
      materialLabSection.classList.add('hidden');
      rightPanelEl?.classList.remove('material-context');
      document.body.classList.remove('material-panel-active');
      // Restore the bone-reveal button only if the selection actually has a
      // skeleton — otherwise leave it hidden so the light/render toggle
      // (its footer sibling) can take the slot instead.
      checkSceneForSkeletons();
      if (bonePanelBeforeMaterial) {
        bonePanelBeforeMaterial = false;
        // Only restore the bone panel if the current selection still
        // actually has a skeleton (it may have changed while Material
        // was open) — otherwise just collapse the split like normal.
        if (currentSkeleton && typeof openBonePanel === 'function') openBonePanel();
        else objectsSectionEl.classList.remove('split');
      } else {
        objectsSectionEl.classList.remove('split');
      }
    }
  }
