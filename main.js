...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  function toggleRenderPreview() {
    ...
    renderLightToggleBtn?.classList.toggle('active', renderPreviewActive);
    setRenderPreviewMode(renderPreviewActive);
  }
  renderPreviewToggle?.addEventListener('click', toggleRenderPreview);

  // Same action as the "Modo Render" button above, but living in the bone
  // button's footer slot (visible only when there's no skeleton to show —
  // see #boneRevealBtn.hidden ~ #renderLightToggleBtn in ui.css) so there's
  // always a one-tap way to preview the render right there in the viewport.
  renderLightToggleBtn?.addEventListener('click', toggleRenderPreview);
