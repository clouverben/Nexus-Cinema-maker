  ...
  async function importModel(file) {
    ...
    loader.parse(arrayBuffer, '', (gltf) => {
      ...
      if (gltf.animations?.length) {
        ...
        const wants = confirm(
          `Esse modelo tem ${n} animação${n > 1 ? 'ões' : ''} embutida${n > 1 ? 's' : ''}. Importar?\n\n` +
          `Elas vão aparecer numa "Biblioteca de Animações" no final do painel Animação — você clica numa pra selecionar e confirma pra adicionar ela no objeto.`
        );
