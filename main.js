...
import { ensureReflectionConfig, ssrSettings, syncSSRSettings } from './reflections.js';
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  function syncPanelExtension() {
    ...
    if (isMaterial) syncReflectionsUI();
  }
  ...
  // ── Reflections UI (inside the Material extension) ─────────────────────────
  const reflCubeEnabled      = document.getElementById('reflCubeEnabled');
  const reflCubeFields       = document.getElementById('reflCubeFields');
  const reflCubeResolution   = document.getElementById('reflCubeResolution');
  const reflCubeIntensity    = document.getElementById('reflCubeIntensity');
  const reflCubeNear         = document.getElementById('reflCubeNear');
  const reflCubeFar          = document.getElementById('reflCubeFar');
  const reflCubeUpdateEvery  = document.getElementById('reflCubeUpdateEvery');
  const reflSSREnabled       = document.getElementById('reflSSREnabled');
  const reflSSRUnsupported   = document.getElementById('reflSSRUnsupported');
  const reflSSRFields        = document.getElementById('reflSSRFields');
  ...
  function _reflectableMesh() {
    return _resolveMesh(app.selected);
  }

  function syncReflectionsUI() {
    const mesh = _reflectableMesh();
    reflCubeEnabled.disabled = !mesh;
    reflSSREnabled.disabled  = !mesh;
    if (!mesh) {
      reflCubeEnabled.checked = false;
      reflSSREnabled.checked  = false;
      reflCubeFields.classList.add('hidden');
      reflSSRFields.classList.add('hidden');
      return;
    }
    const refl = ensureReflectionConfig(mesh);
    reflCubeEnabled.checked = !!refl.cubeCamera.enabled;
    reflCubeFields.classList.toggle('hidden', !refl.cubeCamera.enabled);
    reflCubeResolution.value  = refl.cubeCamera.resolution;
    reflCubeIntensity.value   = refl.cubeCamera.intensity;
    reflCubeNear.value        = refl.cubeCamera.near;
    reflCubeFar.value         = refl.cubeCamera.far;
    reflCubeUpdateEvery.value = refl.cubeCamera.updateEvery;

    reflSSREnabled.checked = !!refl.ssrEnabled;
    reflSSRFields.classList.toggle('hidden', !refl.ssrEnabled);
    reflSSRUnsupported.classList.toggle('hidden', isLightPreviewActive());
    // SSR settings are global (one shared pass for the whole scene) — show
    // the current shared values regardless of which object is selected.
    reflSSROpacity.value         = ssrSettings.opacity;
    reflSSRMaxDistance.value     = ssrSettings.maxDistance;
    reflSSRThickness.value       = ssrSettings.thickness;
    reflSSRResolutionScale.value = ssrSettings.resolutionScale;
    reflSSRBlur.checked          = !!ssrSettings.blur;
  }

  reflCubeEnabled.addEventListener('change', () => {
    const mesh = _reflectableMesh(); if (!mesh) return;
    ensureReflectionConfig(mesh).cubeCamera.enabled = reflCubeEnabled.checked;
    reflCubeFields.classList.toggle('hidden', !reflCubeEnabled.checked);
    markSceneDirty();
  });
  function _bindCubeField(input, key) {
    input.addEventListener('input', () => {
      const mesh = _reflectableMesh(); if (!mesh) return;
      ensureReflectionConfig(mesh).cubeCamera[key] = Number(input.value);
      markSceneDirty();
    });
  }
  _bindCubeField(reflCubeResolution,  'resolution');
  _bindCubeField(reflCubeIntensity,   'intensity');
  _bindCubeField(reflCubeNear,        'near');
  _bindCubeField(reflCubeFar,         'far');
  _bindCubeField(reflCubeUpdateEvery, 'updateEvery');

  reflSSREnabled.addEventListener('change', () => {
    const mesh = _reflectableMesh(); if (!mesh) return;
    ensureReflectionConfig(mesh).ssrEnabled = reflSSREnabled.checked;
    reflSSRFields.classList.toggle('hidden', !reflSSREnabled.checked);
    markSceneDirty();
    refreshReflectionsPipeline(); // structural: this object enters/leaves the shared SSR pass's selects list
  });
  ...
  let   boneVizGroups      = [];     // one Group (white sphere) per bone

  // ── Build / clear white-sphere bone visualization ─────────────────────────
  // One small white sphere per bone, positioned at its joint (local origin)
  // and parented directly to it — THREE's normal scene-graph matrixWorld
  // propagation then makes it follow animated poses automatically. No
  // per-frame sync function needed. setSelected() (scene.js) recolors the
  // sphere orange on selection and back to white on deselect.
  ...
  function buildBoneVisualization(skeleton) {
    ...
    import('three').then(({
      MeshBasicMaterial, Mesh, SphereGeometry, Group,
    }) => {
      const bones = skeleton.bones;

      // Sphere radius scales with this rig's actual bone length (average
      ...
      const radius = Math.max(avgLen * 0.16, 0.01);
  ...
  document.getElementById('actionDelete').addEventListener('click', () => {
    ...
    refreshReflectionsPipeline(); // in case an object with SSR enabled was just deleted
