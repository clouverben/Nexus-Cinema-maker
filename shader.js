...
export const renderState = {
  ...
  post: {
    ...
    saturation: 1,
    vignette:   0,
    // Renderer-level tone mapping — a different, earlier step than the
    // `exposure`/`contrast`/etc. grading above: this runs on the raw HDR
    // linear scene light BEFORE it's compressed into displayable 0-1 color,
    // while the grading fields above tweak the image that comes out the
    // other end of that curve.
    toneMapping:         'Linear', // 'None'|'Linear'|'Reinhard'|'Cineon'|'ACESFilmic'|'AgX'|'Neutral'
    toneMappingExposure: 1,
    colorSpace:          'Linear' // 'sRGB'|'Linear'
  },
  ...
  path: {
    ...
    lowResScale:        0.18,
    renderScale:        0.85,
    // Adaptive mobile path-tracing profile. `autoQuality` changes only
    // runtime quality knobs; the user's manual values remain available when
    // Auto is disabled. These fields are serialized with the render state.
    autoQuality:        true,
    targetFPS:          30,
    minRenderScale:     0.5,
    maxRenderScale:     0.9,
...
export function setToneMappingValue(app, name) {
  renderState.post.toneMapping = name;
  if (app?.renderer) {
    app.renderer.toneMapping = TONE_MAPPING_MAP[name] ?? THREE.ACESFilmicToneMapping;
  }
}

export function setToneMappingExposureValue(app, value) {
  const v = Number(value);
  renderState.post.toneMappingExposure = Number.isFinite(v) ? Math.max(0, v) : 1;
  if (app?.renderer) {
    app.renderer.toneMappingExposure = renderState.post.toneMappingExposure;
  }
}
...
export function setColorSpaceValue(app, name) {
  renderState.post.colorSpace = name;
  if (app?.renderer) {
    app.renderer.outputColorSpace = COLOR_SPACE_MAP[name] ?? THREE.SRGBColorSpace;
  }
}
