...
function _applyReflectionsState(reflData) {
  if (!reflData) return;
  Object.assign(ssrSettings, reflData);
  syncSSRSettings();
  const checkbox = document.getElementById('reflSSREnabled');
  if (checkbox) checkbox.checked = !!ssrSettings.enabled;
  if (ssrSettings.enabled) refreshReflectionsPipeline();
}
