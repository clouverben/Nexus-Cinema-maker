...
import { outlineConfig, setOutlineEnabled, setOutlineColor, refreshGodRaysPipeline } from './posprocess.js';
...
export function _serialize() {
  ...
  return {
    ...
    render: {
      ...
      reflection: {
        opacity: ssrSettings.opacity,
        maxDistance: ssrSettings.maxDistance,
        thickness: ssrSettings.thickness,
        resolutionScale: ssrSettings.resolutionScale,
        blur: ssrSettings.blur,
      },
    },
...
export async function _applyProjectData(data) {
  ...
  // 3g. Restaurar reflexão global do painel Render (sem CubeCamera e sem
  // toggle de ativação — a ativação acompanha o botão lâmpada).
  try {
    const r = data.render?.reflection;
    if (r) {
      if (Number.isFinite(Number(r.opacity))) ssrSettings.opacity = Number(r.opacity);
      if (Number.isFinite(Number(r.maxDistance))) ssrSettings.maxDistance = Number(r.maxDistance);
      if (Number.isFinite(Number(r.thickness))) ssrSettings.thickness = Number(r.thickness);
      if (Number.isFinite(Number(r.resolutionScale))) ssrSettings.resolutionScale = Number(r.resolutionScale);
      if (typeof r.blur === 'boolean') ssrSettings.blur = r.blur;
      syncSSRSettings();
    }
  } catch (e) { console.warn('[config] Erro ao restaurar reflexão:', e); }

  // 3h. Restaurar o Material Lab / estado compartilhado do PBR
...
async function _restoreObject(entry, scene) {
  ...
  if (primName) {
    ...
    _applyReflection(mesh, entry.reflection);
