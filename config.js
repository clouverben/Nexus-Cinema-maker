...
import { outlineConfig, setOutlineEnabled, setOutlineColor, refreshGodRaysPipeline, refreshReflectionsPipeline } from './posprocess.js';
import { addLight } from './lights.js';
import { GOD_RAYS_DEFAULTS } from './godrays.js';
import { ssrSettings, syncSSRSettings } from './reflections.js';
import { serializeCameras, restoreCameras } from './cameras.js';
import { resolveEffectTarget, serializeOwnEffect, restoreOwnEffects } from './ShaderEffectManager.js';
import { renderState } from './shader.js';
import { getRenderPassState, applyRenderPassState } from './render-passes.js';
import { applyProceduralPBRToMaterial, getProceduralPBRState, applyProceduralPBRState } from './procedural-pbr.js';
...
// Reused by the Objects panel's Presets feature (main.js) — same toJSON +
// embedded-texture + size-guard logic used to save imported models into the
// project file, just exposed for saving a single object/subtree on its own.
export function serializeObjectForPreset(obj) {
  return _serializeModel(obj);
}
...
function _objectPath(root, target) {
  if (!root || !target || root === target) return [];
  const path = [];
  let current = target;
...
function _findByObjectPath(root, path) {
  let current = root;
  for (const index of (path || [])) current = current?.children?.[index] ?? null;
  return current || null;
}
function _serializeMorphTargets(obj) {
  const out = [];
  obj.traverse(child => {
    if ((!child.isMesh && !child.isSkinnedMesh) || !child.morphTargetDictionary || !child.morphTargetInfluences) return;
    const values = {};
    Object.entries(child.morphTargetDictionary).forEach(([name, index]) => { values[name] = Number(child.morphTargetInfluences[index]) || 0; });
    if (Object.keys(values).length) out.push({ path: _objectPath(obj, child), values });
  });
  return out;
}
function _restoreMorphTargets(root, data = []) {
  (data || []).forEach(item => {
    const target = _findByObjectPath(root, item.path);
    ...
    Object.entries(item.values || {}).forEach(([name, value]) => {
      ...
      if (index !== undefined) target.morphTargetInfluences[index] = Number(value) || 0;
    });
  });
}
...
export function _serialize() {
  ...
  scene.children.filter(_isUserObject).forEach(obj => {
    try {
      const entry = {
        ...
        visible:  obj.visible,
        // userData pode conter referências de runtime (ex.: lightRef/modelRef,
        // helpers e outros objetos Three.js). Essas referências não podem ser
        // clonadas pelo IndexedDB e faziam o save do projeto inteiro falhar.
        userData: (() => {
          const ud = { ...obj.userData };
          delete ud.lightRef;
          delete ud.modelRef;
          delete ud.selectTarget;
          delete ud.boneRef;
          delete ud.wireRef;
          delete ud.__hl;
          return ud;
        })(),
        morphTargets: _serializeMorphTargets(obj),
        shaderState: (() => {
          try {
            const target = resolveEffectTarget(obj);
            const state = target ? serializeOwnEffect(target) : { key: 'none', params: {} };
            return state;
          } catch { return { key: 'none', params: {} }; }
        })(),
      };
      ...
      if (mesh?.material) {
        ...
        entry.material = {
          ...
          side:              m.side,
          proceduralPBR:      m.userData?.ncmProceduralPBR || null,
        };
      }
      ...
      if (obj.userData?.isLightObject && obj.userData?.lightRef) {
        ...
        entry.light = {
          lightType:  obj.userData.lightType || l.type,
          color:      l.color.getHex(),
          intensity:  l.intensity,
          distance:   l.distance ?? 0,
          castShadow: l.castShadow,
          godRays:    l.userData?.godRays ? { ...l.userData.godRays } : null,
        };
      }
  ...
  // ── Aura systems — same fix as particleSystems above: Aura is a fully
  // separate module (auraLabs.js) from Particle Labs, so it needs its own
  // line here or it silently disappears on normal project save/exit (it
  // would only have survived via Particle Labs' own standalone .nex
  // export, which most people don't use as their main save path).
  let auraSystems = [];
  try { auraSystems = window._nexusAuraLab?.serializeAuras?.() ?? []; }
  catch (e) { console.warn('[config] Erro ao serializar auras:', e); }

  // ── Lightning (Particle Labs) — same AuraLab module, tracked in its own
  // list (see AuraLab.createLightning) since it lives in the normal
  // Sistema tab instead of behind the Aura Labs segment.
  let lightningSystems = [];
  try { lightningSystems = window._nexusAuraLab?.serializeLightnings?.() ?? []; }
  catch (e) { console.warn('[config] Erro ao serializar lightning:', e); }
  ...
  return {
    ...
    auraSystems,
    lightningSystems,
    // Render configuration is part of the project so Path Tracer quality
    // settings (including the mobile Auto profile) survive .nex save/load.
    render: {
      mode: renderState.mode,
      samples: { ...renderState.samples },
      path: { ...renderState.path },
      ray: { ...renderState.ray },
      bloom: { ...renderState.bloom },
      post: { ...renderState.post },
      renderPasses: getRenderPassState(),
      reflections: { ...ssrSettings },
    },
    proceduralPBR: getProceduralPBRState(),
    animation: {
      fps:        animFps,
      interp:     animInterp,
...
export async function _applyProjectData(data) {
  ...
  // 3d. Restaurar sistemas de aura
  try { window._nexusAuraLab?.restoreAuras?.(data.auraSystems || []); }
  catch (e) { console.warn('[config] Erro ao restaurar auras:', e); }

  // 3f. Restaurar configurações de Render Pass / AOV
  try { applyRenderPassState(data.render?.renderPasses || data.renderPasses || {}); }
  catch (e) { console.warn('[config] Erro ao restaurar Render Pass:', e); }

  // 3f-2. Restaurar Reflexos SSR (configuração global do painel Render)
  try { _applyReflectionsState(data.render?.reflections); }
  catch (e) { console.warn('[config] Erro ao restaurar Reflexos SSR:', e); }

  // 3g. Restaurar o Material Lab / estado compartilhado do PBR
  try { applyProceduralPBRState(data.proceduralPBR || {}); }
  catch (e) { console.warn('[config] Erro ao restaurar estado Procedural PBR:', e); }

  // 3e. Restaurar sistemas de lightning
  try { window._nexusAuraLab?.restoreLightnings?.(data.lightningSystems || []); }
  catch (e) { console.warn('[config] Erro ao restaurar lightning:', e); }
  ...
  setTimeout(() => {
    ...
    // Build stable fallbacks for bones and top-level objects. Newer projects
    // restore the original Object3D.uuid directly; older projects did not, so
    // name matching keeps their animation tracks recoverable as well.
    const boneNameToUUID = {};
    const objectNameToUUID = {};
    scene.traverse(o => {
      if (o.isBone && o.name) boneNameToUUID[o.name] = o.uuid;
      if (_isUserObject(o) && o.name && !objectNameToUUID[o.name]) objectNameToUUID[o.name] = o.uuid;
    });
    const savedObjectNameByUUID = {};
    (data.objects || []).forEach(entry => { if (entry?.uuid && entry?.name) savedObjectNameByUUID[entry.uuid] = entry.name; });
    ...
    Object.entries(animData.keyframes).forEach(([savedUUID, frames]) => {
      ...
      if (!directObj) {
        ...
        if (boneName && boneNameToUUID[boneName]) newUUID = boneNameToUUID[boneName];
          ...
          const objName = savedObjectNameByUUID[savedUUID];
          if (objName && objectNameToUUID[objName]) newUUID = objectNameToUUID[objName];
        }
      }
...
async function _restoreObject(entry, scene) {
  ...
  if (entry.userData?.isLightObject && entry.light) {
    ...
    if (!group) return;
    if (entry.uuid && !scene.getObjectByProperty('uuid', entry.uuid)) group.uuid = entry.uuid;
    ...
    if (light) {
      ...
      if (entry.light.godRays) {
        light.userData.godRays = { ...GOD_RAYS_DEFAULTS, ...entry.light.godRays };
      }
    }
  ...
  if (entry.userData?.isImportedModel && entry.modelData) {
    try {
      ...
      if (entry.uuid && !scene.getObjectByProperty('uuid', entry.uuid)) restored.uuid = entry.uuid;
      ...
      _restoreMorphTargets(restored, entry.morphTargets || []);
      try {
        const target = resolveEffectTarget(restored);
        if (target && entry.shaderState) restoreOwnEffects(target, entry.shaderState);
      } catch (e) { console.warn('[config] Shader do modelo não restaurado:', e); }
    } catch (e) {
  ...
  const PRIMITIVES = ['box','sphere','cylinder','cone','torus','plane','capsule','icosahedron','octahedron','tetrahedron','dodecahedron','torusKnot','ring','circle','triangularPrism','pyramid'];
  ...
  if (primName) {
    ...
    if (!mesh) return;
    if (entry.uuid && !scene.getObjectByProperty('uuid', entry.uuid)) mesh.uuid = entry.uuid;
    ...
    _restoreMorphTargets(mesh, entry.morphTargets || []);
    try { if (entry.shaderState) restoreOwnEffects(mesh, entry.shaderState); }
    catch (e) { console.warn('[config] Shader da primitiva não restaurado:', e); }
    return;
  }
  ...
  if (entry.type === 'Mesh' || entry.type === 'SkinnedMesh') {
    ...
    try { if (entry.shaderState) restoreOwnEffects(mesh, entry.shaderState); }
    catch (e) { console.warn('[config] Shader do mesh não restaurado:', e); }
    return;
  }
  ...
  if (entry.type === 'Group') {
    ...
    if (entry.uuid && !scene.getObjectByProperty('uuid', entry.uuid)) grp.uuid = entry.uuid;
    grp.name = entry.name;
    ...
    try {
      const target = resolveEffectTarget(grp);
      if (target && entry.shaderState) restoreOwnEffects(target, entry.shaderState);
    } catch (e) { console.warn('[config] Shader do grupo não restaurado:', e); }
  }
}
...
function _applyMaterial(mesh, matData) {
  ...
  m.needsUpdate = true;
  if (matData.proceduralPBR?.maps) {
    try { applyProceduralPBRToMaterial(mesh, matData.proceduralPBR); } catch (e) { console.warn('[config] Procedural PBR restore failed:', e); }
  }
}

// SSR is a global render setting now (Render panel → Pós-proc.), not a
// per-object property — restore it once for the whole project instead of
// per-mesh. See _applyProjectData() for where this gets called.
function _applyReflectionsState(reflData) {
  if (!reflData) return;
  // Keep legacy project files compatible, but never restore the old
  // activation toggle. SSR is always available to the Render pipeline;
  // these settings only control how it behaves.
  const { enabled: _legacyEnabled, ...numericSettings } = reflData;
  Object.assign(ssrSettings, numericSettings);
...
class SkyboxManager {
  ...
  setSolidColor(hex) {
    ...
    // Falls back to the procedural studio environment (scene.js) rather
    // than null, so PBR materials keep their soft IBL fill/reflections
    // even with a flat background color — only a real loaded HDRI (below)
    // should ever fully replace it.
    this.scene.environment = app.defaultEnvironment ?? null;
    markSceneDirty();
  }
...
export async function initConfig(_app) {
  ...
  const rightPanelWorkspace = document.getElementById('rightPanelWorkspace');
  ...
  function setLabsActive(on) {
    ...
    // The whole Objects/Material Lab workspace has to step aside while
    // Labs is open — leaving it in place (even with objectsSection hidden)
    // left it as an empty flex sibling competing for height with
    // #labsSection, squashing the Labs panel into half the available space.
    if (rightPanelWorkspace) rightPanelWorkspace.classList.toggle('hidden', _labsActive);
    objectsSection.classList.toggle('hidden', _labsActive);
    labsSection.classList.toggle('hidden', !_labsActive);
    if (rightPanelTitle) rightPanelTitle.textContent = _labsActive ? 'Labs' : 'Objetos';
  ...
  // Exposed globally — the Settings menu's "Entrar no Labs" row (behind a
  // confirmation modal — see index.html) calls this, along with main.js's
  // undo/redo-triggered UI refreshes.
  ...
  window._openInfoSection = function() {
    _infoOpen = true;
    if (rightPanelWorkspace) rightPanelWorkspace.classList.add('hidden');
  ...
  if (axesToggle) {
    const initial = _persisted.axesVisible ?? false;
  ...
  // Contorno de seleção — visibilidade e cor
  const outlineToggle = document.getElementById('cfgOutlineVisible');
  if (outlineToggle) {
    const initial = _persisted.outlineVisible ?? true;
    outlineToggle.checked = initial;
    setOutlineEnabled(initial);
    outlineToggle.addEventListener('change', (e) => {
      setOutlineEnabled(e.target.checked);
      _saveSetting('outlineVisible', e.target.checked);
    });
  }
  const outlineColorInput = document.getElementById('cfgOutlineColor');
  if (outlineColorInput) {
    const initial = _persisted.outlineColor ?? outlineConfig.color;
    outlineColorInput.value = initial;
    setOutlineColor(initial);
    outlineColorInput.addEventListener('input', (e) => {
      setOutlineColor(e.target.value);
      _saveSetting('outlineColor', e.target.value);
    });
  }
  ...
  if (skyRemoveBtn) skyRemoveBtn.addEventListener('click', () => {
    skybox.remove();
    if (bgColorInput) bgColorInput.value = '#373737';
    _toast('Skybox removido.');
  });

  // Ir para a tela de Projetos
  const goToProjectsBtn = document.getElementById('cfgGoToProjects');
  if (goToProjectsBtn) goToProjectsBtn.addEventListener('click', async () => {
    document.getElementById('settingsMenu')?.classList.add('hidden');
    document.getElementById('settingsMenuBackdrop')?.classList.add('hidden');
    document.getElementById('settingsBtn')?.classList.remove('active');
    // Force a save of whatever's live right now BEFORE tearing down the
    // editor. Some edits (animation keyframes especially — see
    // animation.js) don't always fire the scene-dirty event the normal
    // debounced autosave waits for, so without this, work done in the
    // last few seconds before leaving could be silently lost.
    goToProjectsBtn.disabled = true;
    try {
      await window.NCMLauncher?.autosave?.();
    } finally {
      goToProjectsBtn.disabled = false;
    }
    window.NCMLauncher?.show?.();
  });
