...
import { ensureReflectionConfig } from './reflections.js';
    ...
    try {
      ...
      if (mesh?.material) {
        ...
        if (mesh.userData?.reflection) {
          entry.reflection = {
            cubeCamera: { ...mesh.userData.reflection.cubeCamera },
            ssrEnabled: !!mesh.userData.reflection.ssrEnabled,
          };
        }
      }
...
function _applyReflection(mesh, reflData) {
  if (!reflData) return;
  const refl = ensureReflectionConfig(mesh);
  if (reflData.cubeCamera) Object.assign(refl.cubeCamera, reflData.cubeCamera);
  refl.ssrEnabled = !!reflData.ssrEnabled;
  if (refl.ssrEnabled) refreshReflectionsPipeline();
}
