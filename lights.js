...
import { ensureGodRaysConfig } from './godrays.js';

let rectAreaInitialized = false;

// ─────────────────────────────────────────────────────────────────────────────
// 3D light viewport models — real meshes, never camera-facing sprites.
// The shapes are solid (no wireframe) so they read clearly at a glance, and
// each model carries a small arrow underneath pointing in the direction the
// light travels — quick visual feedback for where it's coming from / going.
// ─────────────────────────────────────────────────────────────────────────────
...
// Small shaft + arrowhead pointing straight down (local -Y) from the model,
// showing the direction the light is emitted/traveling. `dropOffset` is how
// far below the group's origin the model's visible geometry already ends,
// so the arrow starts right at that edge instead of poking through it.
function addDirectionArrow(group, color, dropOffset = 0.34, length = 0.4) {
  const shaftLen = length * 0.7;
  const headLen = length - shaftLen;
  const arrow = new THREE.Group();
  arrow.name = 'LightDirectionArrow';

  const shaft = new THREE.Mesh(
    new THREE.CylinderGeometry(0.016, 0.016, shaftLen, 8),
    makeLightMaterial(color, 0.85)
  );
  shaft.position.y = -dropOffset - shaftLen / 2;
  arrow.add(shaft);

  const head = new THREE.Mesh(
    new THREE.ConeGeometry(0.045, headLen, 10),
    makeLightMaterial(color, 0.95)
  );
  head.position.y = -dropOffset - shaftLen - headLen / 2;
  arrow.add(head);
...
function makePointModel(color) {
  ...
  addDirectionArrow(g, color, 0.33);
...
function makeSunModel(color) {
  ...
  addDirectionArrow(g, color, 0.40);
...
function makeSpotModel(color) {
  ...
  addDirectionArrow(g, color, 0.58);
...
function makeRimModel(color) {
  ...
  addDirectionArrow(g, color, 0.47);
...
function makeAreaModel(color) {
  ...
  g.add(edges);
  addDirectionArrow(g, color, 0.09);
...
export function addLight(type) {
  ...
  ensureGodRaysConfig(light); // Point/Area never read it (see godrays.js) — harmless to attach either way.
