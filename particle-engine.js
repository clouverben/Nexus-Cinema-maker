...
export class ParticleLab {
    ...
    // User-saved presets (see index.html's "Meus Presets" picker) — config
    // is just a snapshot of some ParticleSystem's own getConfig() output,
    // so this is really the same as createFromPreset above minus the
    // burst-on-preset-definition special case, since a saved snapshot
    // doesn't carry that flag the way a curated PARTICLE_PRESETS entry can.
    createFromCustomPreset(name, config, position = new THREE.Vector3(0, 1, 0)) {
        const cfg = JSON.parse(JSON.stringify(config || {}));
        if (cfg.direction) cfg.direction = new THREE.Vector3(cfg.direction.x, cfg.direction.y, cfg.direction.z);
        const sys = new ParticleSystem(this._scene, { ...cfg, name: name || 'Preset' });
        sys.position.copy(position);
        if (sys._marker) sys._marker.position.copy(position);
        this._registerSystem(sys);
        sys.play();
