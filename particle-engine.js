...
function _makeSmokeTex() {
    const c = document.createElement('canvas'); c.width=c.height=64;
    const ctx=c.getContext('2d');
    // FIX: was 0.25 max alpha per blob with little overlap → nearly
    // invisible. Boosted alpha + bigger/denser blobs so it actually reads
    // as a puffy cloud instead of a faint grey smudge.
    for(let i=0;i<10;i++){
...
function _makeStreakTex() {
    // Thin vertical streak — used for rain/falling-water motion blur (was
    // confusingly named "lightning" despite not being bolt-shaped at all;
    // see _makeBoltTex() below for an actual lightning-bolt silhouette).
...
function _makeSquareTex() {
    // FIX: this used to fill the square with a soft radial gradient whose
    // radius was smaller than the square's own corners, so the hard edges
    // never showed up at all — it just read as another blurry circle
    // (exactly the "everything is a glowing ball" complaint). Confetti
    // needs to actually look like a flat paper chip: solid fill, crisp
    // edges, only a subtle diagonal sheen for a bit of dimension.
...
function _makeDiamondTex() {
    // Same fix as _makeSquareTex above — solid rotated-square fill instead
    // of an undersized radial gradient, so it actually reads as a faceted
    // diamond silhouette instead of a soft circle.
    const c=document.createElement('canvas'); c.width=c.height=32;
    const ctx=c.getContext('2d');
...
function _makeHexTex() {
    // FIX: the fill gradient's alpha was already ~0 by the time it reached
    // the hexagon's straight edges, so the 6-sided silhouette never showed
    // — same "just a soft circle" problem as square/diamond above. Tighter
    // gradient + a bright rim stroke so the hex outline is unmistakable.
    const c=document.createElement('canvas'); c.width=c.height=64;
    const ctx=c.getContext('2d');
...
function _makePetalTex() {
    // Cherry-blossom petal — rounded body tapering to a base point, with the
    // characteristic shallow notch/cleft at the outer tip (real sakura
    // petals aren't smooth ovals — this is what actually reads as "sakura"
    // instead of a generic diamond sparkle).
...
function _makeBoltTex() {
    // Classic zigzag lightning-bolt silhouette (⚡) — for scattered
    // lightning-strike burst particles. Distinct from the thin
    // electric-arc streamer texture used by the beam renderer.
    const c=document.createElement('canvas'); c.width=40; c.height=64;
    const ctx=c.getContext('2d'); ctx.clearRect(0,0,40,64);
    ctx.save(); ctx.translate(20,32);
    ctx.beginPath();
...
function _makeCoinTex() {
    // Flat coin — a filled disc (not a hollow ring, which reads as a
    // bracelet/portal instead) with a rim and a curved glint.
    const c=document.createElement('canvas'); c.width=c.height=64;
    const ctx=c.getContext('2d'); ctx.clearRect(0,0,64,64);
...
// ── Object/Mesh renderer support — extrudes the SAME vector path each
// texture's 2D sprite is drawn from into a real 3D shape, instead of a
// generic primitive (box/sphere/etc). Coordinates below are lifted
// directly from each _makeXTex() function above (with y negated, since
// canvas Y grows downward but THREE.Shape's Y grows upward like normal
// math), so "bolt" gives an actual 3D lightning-bolt prism, "heart" an
// actual 3D heart, "hex" an actual hexagonal prism, etc. — a real 3D
// version of the shape the person already picked, not an unrelated cube.
function _buildExtrudeShape(textureName) {
    ...
    switch (textureName) {
        ...
        case 'coin': case 'orb': case 'ring':
            ...
            break;
        default:
            // Soft/blob textures with no hard silhouette (glow, smoke,
            // ember, fireball, plasma, cloud, streak, electric...) — a
            // disc is the closest 3D analog to a radial-gradient blob.
...
export const MODULE_LIBRARY = {
    emitters: [
        _mod('emit_continuous', 'Contínuo', '💧', 'emitters',
            ...
            { group: 'emitMode', desc: 'Emite partículas continuamente a uma taxa fixa. "Máscara de Ruído" varia a taxa com um ruído 3D ao longo do espaço/tempo — o equivalente do NCM pra "Texture Influence" do Blender (textura mais clara = emite mais), já que aqui não existe malha com UV pra pintar peso de emissão.',
    ...
    initializers: [
        ...
        // Roblox character-aura technique confirmed via devforum posts on
        // "aura equipping systems": several small Attachments placed at
        // actual body points (head, shoulders, hands, feet), each emitting
        // on its own, instead of one sphere at the character's center.
        // Works as a standalone humanoid approximation — no skeleton/rig
        // required underneath.
        _mod('shape_bodypoints', 'Pontos do Corpo', '🧍', 'initializers',
            ...
            desc: 'Nasce espalhado em 8 pontos de um corpo humanoide (cabeça, ombros, mãos, pés) — técnica de aura de personagem do Roblox, em vez de uma esfera única no centro.' }),
        ...
        // ── Page 2 "Shape" — genuinely new shapes/refinements, not a
        // re-skin of the 8 shapes above (kept in the same 'shape' group
        // so they're still mutually exclusive with them, exactly like SFM).
        _mod('shape_box', 'Caixa', '📦', 'initializers',
            [{ key: 'emitRadius', label: 'Metade do Lado', min: 0, max: 10, step: 0.05, default: 0.5 }], { group: 'shape',
            onApply: c => { c.emitShape = 'box'; }, desc: 'Nasce dentro de um volume cúbico (estilo Roblox "Box").' }),
        ...
        _mod('shape_direction', 'Direção de Emissão', '➡', 'initializers',
            ...
            { desc: 'Usa o próprio formato do emissor para apontar a velocidade inicial — "Fora" vira explosão, "Dentro" vira implosão.' }),
        _mod('shape_arc', 'Arco Parcial', '📐', 'initializers',
            [{ key: 'shapeArc', label: 'Arco (°)', min: 0, max: 360, step: 1, default: 360 }],
            { desc: 'Restringe Anel/Disco/Cone/Cilindro a uma fatia em vez do círculo completo.' }),
    ],
    ...
    operators: [
        ...
        // ── Page 2 "Emitter" — timing window + sub-emission on death ────
        ...
        _mod('emit_on_death', 'Emitir ao Morrer', '💥', 'operators',
            ...
            { desc: 'Ao morrer, cada partícula gera N partículas em cada subpartícula (aba Hierarquia) na posição da morte — ex: rastro que termina em explosão.' }),
        // ── Page 2 "Motion" — genuinely new: existing forces above never
        // reference the emitter's OWN motion, only the particle's.
        _mod('velocity_inheritance', 'Herança de Velocidade', '🧲', 'operators',
            ...
            { desc: 'Partículas novas herdam parte da velocidade atual do próprio emissor — essencial quando ele está Atracado a um objeto em movimento.' }),
        // ── Page 2 "Appearance" — sprite-sheet / flipbook animation ──────
        // Blender's Particles > Children panel: renders extra interpolated
        // points between real ("parent") particles for cheap density —
        // hair thickness, spark clutter — without simulating more real
        // particles. Fills whatever spare room is left in this system's
        // own particle budget; see _update_buffers()'s children block.
        _mod('children_density', 'Filhos (Densidade)', '👨‍👩‍👧', 'operators',
            ...
            { desc: 'Preenche pontos extras interpolados entre partículas reais — densidade de cabelo/faísca mais barata, sem simular mais partículas de verdade. Usa o espaço sobrando no limite de partículas do sistema.' }),
        _mod('flipbook', 'Sprite Sheet / Flipbook', '🎞', 'operators',
            ...
            { onApply: c => { c.flipbookEnabled = true; }, onReset: c => { c.flipbookEnabled = false; },
              desc: 'Anima a textura como uma grade de quadros (use com uma imagem importada) — respeita a vida da partícula.' }),
        // ── Keyframe Inicial / Keyframe Final — limita a emissão contínua a
        // uma janela de frames na timeline compartilhada da cena (o mesmo
        // contador que dirige os Keyframes do resto do app, via
        // window.AnimationSystem.getFrame()) em vez do tempo local do
        // próprio sistema de partículas. Ver a checagem "withinKeyframeRange"
        // no update() mais abaixo neste arquivo.
        _mod('keyframe_range', 'Intervalo de Keyframe', '🎬', 'operators',
            ...
            { onApply: c => { c.keyframeRangeEnabled = true; }, onReset: c => { c.keyframeRangeEnabled = false; },
              desc: 'A partícula só começa a ser emitida a partir do Keyframe Inicial e para de ser emitida depois do Keyframe Final, usando a timeline da cena.' }),
    ],
    ...
    forces: [
        ...
        // ── Blender Force Fields research — Magnetic/Harmonic/Charge ──────
        // Magnetic: "Forcefield depends on the speed of the particles" —
        // pushes perpendicular to the particle's own velocity (scaled by
        // its speed), curving the path like a charged particle in a
        // magnetic field, instead of pulling straight toward/away from a
        // point the way Atrair/Repelir and Força Radial already do.
        _mod('force_magnetic', 'Magnético', '🧭', 'forces',
            [{ key: 'magneticForce', label: 'Força', min: -20, max: 20, step: 0.05, default: 0, addDefault: 4 }],
            { desc: 'Empurra perpendicular à própria velocidade da partícula (curva a trajetória, tipo partícula carregada num campo magnético) — diferente de Atrair/Repelir, que puxa direto pra um ponto.' }),
        // Harmonic: "source of this force is the zero point of a harmonic
        // oscillator" — a spring pulling back toward the emitter's origin,
        // naturally overshoots and oscillates instead of settling like
        // Atrair/Repelir does.
        _mod('force_harmonic', 'Harmônico', '🎯', 'forces',
            ...
            { desc: 'Mola puxando de volta pro ponto de origem — passa do ponto e oscila (não simplesmente converge), tipo um elástico.' }),
        // Charge: particle-to-particle interaction ("only influences other
        // charge force fields" — i.e. reacts to nearby particles, not a
        // fixed point). Sampled against a capped number of neighbours per
        // frame for performance instead of full O(n²).
        _mod('force_charge', 'Carga (Partícula-Partícula)', '⚛', 'forces',
            ...
            { desc: 'Partículas empurram/puxam as vizinhas mais próximas (não um ponto fixo) — repele pra espalhar tipo poeira carregada, ou atrai pra formar aglomerados.' }),
    ],
    ...
    renderers: [
        ...
        _mod('render_beam', 'Feixe / Rope (Laser)', '⚡', 'renderers',
            ...
            { group: 'renderMode', onApply: c => { c.rendererMode = 'beam'; },
              desc: 'Conecta partículas numa fita contínua — lasers, raios, chicotes de energia. Combine com o Iniciador "Linha". "Crepitação" adiciona ruído animado por frame pra fita parecer um raio de verdade tremendo, em vez de uma curva parada (a mesma técnica dos módulos de raio do Roblox Studio: pontos de controle com Perlin noise em camadas).' }),
        // Blender's Render panel: "Object/Collection" — instancia um objeto
        // 3D de verdade em cada partícula (pedra, moeda, fragmento) em vez
        // de um sprite 2D sempre de frente pra câmera. Usa
        // MeshStandardMaterial (reage à luz da cena de verdade, ao
        // contrário do sprite, que é auto-iluminado) — pedras/fragmentos
        // ficam muito mais "sólidos" assim.
        _mod('render_mesh', 'Objeto 3D (Malha)', '🧊', 'renderers',
            ...
            { group: 'renderMode', onApply: c => { c.rendererMode = 'mesh'; },
              desc: 'Cada partícula vira um objeto 3D de verdade extrudado a partir do MESMO contorno da textura escolhida (raio vira raio 3D, coração vira coração 3D, hexágono vira hexágono 3D) — não uma primitiva genérica sem relação. Reage à luz da cena, ao contrário do sprite 2D.' }),
    ],
};
...
export const PARTICLE_PRESETS = {
    ...
    // ── Magic Dust — this is the exact base the user's own hand-tuned
    // "fire" grew out of (decoded from their .nex export): tiny particles,
    // long lifetime, tight line-emission column converging on a nearby
    // target point, full rotation, heat shimmer + a slight spiral, and a
    // LOW lightEmission (0.2-0.3, not 1.0) so with the new stricter bloom
    // threshold it reads as a dense, flickering, living cloud of motes
    // instead of a handful of big blown-out static sprites.
    magicDust: {
        label: 'Magic Dust',
        icon:  '🌟',
        category: 'magic',
    ...
    // ── Energy Aura — same "many tiny particles" recipe as Magic Dust
    // above, tuned electric-blue: a narrow rising column of small motes
    // instead of a handful of big plasma-ball sprites.
    energyAura: {
        label: 'Energy Aura',
        icon:  '⚡',
        category: 'magic',
    ...
    // Devil Fruit Aura — the same rising-column fire recipe as the base
    // Fire preset, just recolored red/orange (One Piece "Mera Mera"-style
    // aura rather than a campfire).
    devilFruitAura: {
    ...
    // ── Lightning Strike — main bolt-shaped burst (unchanged in spirit)
    // plus much smaller/denser secondary spark motes mixed in via a wider
    // size range, so it reads as "a strike with debris/sparks" instead of
    // a uniform ring of identical bolt icons.
    lightningStrike: {
        label: 'Lightning Strike',
        icon:  '🌩️',
        category: 'combat',
        // Widened emitRadius (0.2→0.4) so the 90-particle burst doesn't
        // start 100% overlapped — same fusion risk Electric Aura had,
        // just less severe since these fly outward fast (speed 1.5-6) and
        // disperse within a couple frames instead of sitting stacked.
    ...
    // ── Dark Energy — same tiny-particle rising-column recipe, dark
    // violet fading to near-black, so it reads as a wispy, foreboding
    // motion instead of one static purple blob.
    darkEnergy: {
        label: 'Dark Energy',
        icon:  '🌑',
        category: 'elemental',
...
export class ParticleSystem {
    constructor(scene, config = {}) {
        ...
        // ── Aura Shell (Style "Aura: Shell" mode) ─────────────────────────
        // A companion glow mesh, independent from the particle sprites
        // entirely — a Fresnel/rim shader on a sphere wrapped around the
        // emitter's current position (so it automatically rides along with
        // attachTo()/hierarchy just like the crosshair marker does). This
        // is the "shell" aura approach; the alternative "particles" aura
        // approach reuses the existing ring/orbit/vortex stack modules
        // instead (no new geometry needed for that one). Pre-allocated and
        // simply toggled/updated — never rebuilt — matching every other
        // renderer in this class.
        {
            ...
            this._auraMat = new THREE.ShaderMaterial({
                uniforms: {
                    ...
                    // ── Anime/Roblox "flame shell" upgrade (Page 2 "Aura") ──
                    // Roblox/SFM stylized ki auras are almost never a perfectly
                    // smooth glow — the silhouette is broken up into a jagged,
                    // rising flame edge (hand-drawn-looking VFX sheets in SFM;
                    // scrolling noise/beam rigs in Roblox devforum tutorials).
                    // uJagged mixes the ORIGINAL smooth Fresnel glow (0, exact
                    // legacy behaviour — every existing saved aura defaults
                    // here) with a noise-cut flame silhouette (1). uNoiseScale/
                    // uFlickerSpeed control the grain and upward scroll speed
                    // of that noise so it reads as rising ki rather than static.
                    uJagged:      { value: 0.0 },
                ...
                fragmentShader: /* glsl */`
                    ...
                    // Cheap 3D value noise (hash-based, no texture lookup) —
                    // sampled along the shell's own normal so it stays stable
                    // on the surface, scrolled over time for a rising flicker.
                    float _hash(vec3 p){
                    ...
                    void main(){
                        ...
                        // Jagged flame mask: 1.0 = old smooth disc (byte-for-byte
                        // identical output to the pre-Aura-tab shader whenever
                        // uJagged is 0), lower where noise "eats into" the rim.
            ...
            this._auraShell = new THREE.Mesh(shellGeo, this._auraMat);
            this._auraShell.frustumCulled = false;
            this._auraShell.visible = false;
            this._auraShell.userData = { isLab: true, isHelper: true };
            this._scene.add(this._auraShell);
        }

        // ── Aura Billboard (Page 2 "Aura" — independent on/off layer,
        // stackable with Shell/Partículas) ─────────────────────────────────
        // Stylized anime/Roblox "ki" auras (SSJ, Kaioken, awakening VFX in
        // games like Blox Fruits/Anime Fighting Simulator) are usually a
        // handful of flat, camera-facing "flame" cards fanned out around the
        // character rather than a single smooth 3D shell — the same trick
        // SFM's render_animated_sprites (orientation_type 0/1) and Blender's
        // particle billboard_align + "lock axis" use. Each plane below is
        // billboarded entirely in the vertex shader via the built-in
        // cameraPosition uniform (same no-JS-camera-reference trick already
        // used by the Fresnel shell above and the beam/rope renderer further
        // down) — uAlign blends between a full camera-facing billboard
        // (SFM orientation_type 0) and a Y-axis-locked one that stays
        // upright while still turning to face the camera horizontally (SFM
        // orientation_type 1 / Blender's billboard "lock axis") — the
        // classic choice for flame/energy cards so they don't tip over.
        this._auraBillboards    = [];      // THREE.Mesh[], rebuilt when auraBillboardCount changes
        this._auraBillboardGeo  = new THREE.PlaneGeometry(1, 1);
    ...
    setConfig(cfg) {
        ...
        // ── Aura (Page 2 "Aura" — Shell/Combo approach reacts immediately
        //    even while paused; the Particles-only approach just reuses the
        //    normal module stack and needs no special handling here) ─────
        if (cfg.auraMode !== undefined || cfg.auraShellColor !== undefined || cfg.auraShellIntensity !== undefined
            || cfg.auraShellRadius !== undefined || cfg.auraShellHeight !== undefined
            || cfg.auraShellSharpness !== undefined || cfg.auraShellPulseSpeed !== undefined
            || cfg.auraShellJagged !== undefined || cfg.auraShellNoiseScale !== undefined
            || cfg.auraShellFlickerSpeed !== undefined) {
            this._update_aura_shell();
        }
        // ── Aura Billboard (Page 2 "Aura" — independent stackable layer,
        //    same "react immediately even while paused" treatment as Shell
        //    above) ──────────────────────────────────────────────────────
        if (cfg.auraBillboardEnabled !== undefined || cfg.auraBillboardCount !== undefined
            || cfg.auraBillboardAlign !== undefined || cfg.auraBillboardColorBottom !== undefined
            || cfg.auraBillboardColorTop !== undefined || cfg.auraBillboardWidth !== undefined
            || cfg.auraBillboardHeight !== undefined || cfg.auraBillboardRadius !== undefined
            || cfg.auraBillboardIntensity !== undefined || cfg.auraBillboardScrollSpeed !== undefined
            || cfg.auraBillboardJagged !== undefined || cfg.auraBillboardNoiseScale !== undefined
            || cfg.auraBillboardFlickerSpeed !== undefined) {
            this._update_aura_billboards();
        }
    ...
    // ── Aura Shell (Page 2 "Aura" — Shell/Combo approach) ─────────────
    _update_aura_shell() {
        const c = this._config;
        const on = c.auraMode === 'shell' || c.auraMode === 'combo';
        if (this._auraShell) this._auraShell.visible = on;
        if (!on) return;
        const radius = Math.max(0.05, (c.auraShellRadius ?? 12) / 10);
        const height = Math.max(0.2, (c.auraShellHeight ?? 15) / 10);
        this._auraShell.position.copy(this.position);
        this._auraShell.scale.set(radius, radius * height, radius);
        this._auraMat.uniforms.uColor.value.set(c.auraShellColor ?? '#a78bfa');
        this._auraMat.uniforms.uIntensity.value  = Math.max(0, (c.auraShellIntensity ?? 60) / 40);
        this._auraMat.uniforms.uPower.value      = Math.max(0.2, (c.auraShellSharpness ?? 55) / 25);
        this._auraMat.uniforms.uPulseSpeed.value = (c.auraShellPulseSpeed ?? 30) / 25;
        // Flame jaggedness (Page 2 "Aura") — all three default to the
        // neutral/legacy values, so old saves with no auraShellJagged key
        // at all still render as a perfectly smooth glow, byte-for-byte.
        this._auraMat.uniforms.uJagged.value       = Math.max(0, Math.min(1, (c.auraShellJagged ?? 0) / 100));
        this._auraMat.uniforms.uNoiseScale.value   = Math.max(0.1, (c.auraShellNoiseScale ?? 20) / 10);
        this._auraMat.uniforms.uFlickerSpeed.value = Math.max(0, (c.auraShellFlickerSpeed ?? 45) / 40);
        this._auraMat.uniforms.uTime.value       = this._time;
    }

    // ── Aura Billboard (Page 2 "Aura") — independent stackable layer ──
    _makeAuraBillboardMaterial(angleOffset) {
        return new THREE.ShaderMaterial({
            uniforms: {
                uCenter:       { value: new THREE.Vector3() },
                uAngleOffset:  { value: angleOffset },
                uRadius:       { value: 0.22 },
                uAlign:        { value: 1.0 }, // 0 = full camera billboard, 1 = Y-locked
                uWidth:        { value: 0.6 },
                uHeight:       { value: 1.8 },
            ...
            vertexShader: /* glsl */`
                ...
                void main(){
                    vUv = uv;
                    vec3 ringOffset  = vec3(cos(uAngleOffset), 0.0, sin(uAngleOffset)) * uRadius;
                    vec3 worldCenter = uCenter + ringOffset;

                    // GPU billboard (no JS camera reference needed — same
                    // trick as the Fresnel shell/beam ribbon above): uAlign
                    // blends a full camera-facing billboard with one that
                    // stays locked to world-up (flame cards standing upright).
                    ...
                    // position.y is -0.5..0.5 (unit PlaneGeometry) — shift so
                    // the card grows up from its base instead of its middle.
            ...
            fragmentShader: /* glsl */`
                ...
                void main(){
                    vec2 scrolled = vUv + vec2(0.0, -uTime * uScrollSpeed);
                    float n = _noise(scrolled * uNoiseScale) * 0.6
                            + _noise(scrolled * uNoiseScale * 2.3 + 7.0) * 0.4;
                    float flicker = 0.85 + 0.15 * sin(uTime * uFlickerSpeed + vUv.x * 6.0);

                    float vertFade  = smoothstep(1.0, 0.15, vUv.y);
                    float horizFade = pow(clamp(1.0 - abs(vUv.x - 0.5) * 2.0, 0.0, 1.0), 0.6);
                    float smoothShape = vertFade * horizFade;
                    float shape = mix(smoothShape, smoothShape * n * 1.6, clamp(uJagged, 0.0, 1.0));

                    vec3  col   = mix(uColorBottom, uColorTop, vUv.y) * uIntensity;
                    float alpha = clamp(shape * flicker, 0.0, 1.0);
                    gl_FragColor = vec4(col, alpha);
                }
            `,
            transparent: true,
    ...
    // Rebuilds the N billboard planes whenever auraBillboardCount changes —
    // cheap (N is small, 1-8) and self-correcting: called every frame from
    // _update_aura_billboards() below, only actually rebuilds when the
    // count really differs from what's currently on screen.
    _rebuild_aura_billboards(count) {
        this._auraBillboards.forEach(m => { this._scene.remove(m); m.material.dispose(); });
        this._auraBillboards = [];
        for (let i = 0; i < count; i++) {
            const angle = (i / count) * Math.PI * 2;
            const mesh  = new THREE.Mesh(this._auraBillboardGeo, this._makeAuraBillboardMaterial(angle));
            mesh.frustumCulled = false;
            mesh.visible = false;
            mesh.userData = { isLab: true, isHelper: true };
            this._scene.add(mesh);
            this._auraBillboards.push(mesh);
        }
    }

    _update_aura_billboards() {
        const c  = this._config;
        const on = !!c.auraBillboardEnabled;
        const count = Math.max(1, Math.min(8, Math.round(c.auraBillboardCount ?? 4)));
        if (this._auraBillboards.length !== count) this._rebuild_aura_billboards(count);
        this._auraBillboards.forEach(m => { m.visible = on; });
        if (!on) return;

        const align = c.auraBillboardAlign === 'camera' ? 0.0 : 1.0;
        this._auraBillboards.forEach(mesh => {
            const u = mesh.material.uniforms;
            u.uCenter.value.copy(this.position);
            u.uRadius.value       = Math.max(0, c.auraBillboardRadius ?? 0.22);
            u.uAlign.value        = align;
            u.uWidth.value        = Math.max(0.02, c.auraBillboardWidth ?? 0.6);
            u.uHeight.value       = Math.max(0.05, c.auraBillboardHeight ?? 1.8);
            u.uColorBottom.value.set(c.auraBillboardColorBottom ?? '#ff9500');
            u.uColorTop.value.set(c.auraBillboardColorTop ?? '#ffffff');
            u.uIntensity.value    = Math.max(0, (c.auraBillboardIntensity ?? 150) / 100);
            u.uScrollSpeed.value  = c.auraBillboardScrollSpeed ?? 0.6;
            u.uJagged.value       = Math.max(0, Math.min(1, (c.auraBillboardJagged ?? 65) / 100));
            u.uNoiseScale.value   = Math.max(0.5, c.auraBillboardNoiseScale ?? 8);
            u.uFlickerSpeed.value = Math.max(0, c.auraBillboardFlickerSpeed ?? 3);
    ...
    destroy() {
        ...
        if (this._auraShell) {
            this._scene.remove(this._auraShell);
            this._auraShell.geometry?.dispose();
            this._auraMat?.dispose();
        }
        this._auraBillboards.forEach(m => { this._scene.remove(m); m.material.dispose(); });
        this._auraBillboards = [];
        this._auraBillboardGeo?.dispose();
        if (this._marker) {
