...
// Cache uuid → objeto, pra não ter que varrer a cena inteira toda vez que
// alguém precisa achar um objeto (a versão antiga fazia scene.getObjectByProperty,
// que é um traverse completo — durante o play, applyKeyframesAtFrame chama
// isso pra CADA osso, EM TODO frame, então num rig com muitos ossos isso
// virava milhares de traversals/seg e travava a página). Autocura: se o
// objeto guardado não estiver mais preso na cena (saiu por delete/undo), ou
// se o uuid nunca foi visto, refaz a busca ao vivo só daquela vez e
// recacheia — sem precisar de hooks externos toda vez que algo é
// adicionado/removido da cena.
let _uuidCache = null;
...
// ==================== PLAYHEAD ====================
// Durante o play, DopeSheet/Graph Editor/Onion Skin só são re-renderizados
// a cada N frames (não every single frame) — são os três mais caros daqui
// (DOM cheio de diamantes, canvas reamostrando a curva inteira, clonar/
// destruir ghosts 3D) e não precisam de 60fps pra parecerem fluidos; o
// playhead/timecode em si continuam atualizando todo frame, sem serrilhado.
// O Graph Editor em especial fica visível POR PADRÃO, então isso evita que
// qualquer play já comece pesado sem o usuário ter mexido em nada.
...
// Cache do array de frames ordenado por (objKFs), pra não ter que refazer
// Object.keys()+sort() a cada chamada — getInterpolatedKF é chamado
// centenas/milhares de vezes por segundo durante o play (uma vez por osso
// por frame) e pelo Graph Editor (que fica visível por padrão e reamostra
// a curva inteira a cada frame). Num clipe "assado" (um keyframe em CADA
// frame, como os que vêm da Biblioteca de Animações importada), isso virava
// um sort de centenas de itens milhares de vezes por segundo — a causa
// principal do app travar de vez ao dar play. Invalidação por contagem de
// chaves é segura aqui porque não existe "mover keyframe" nesse app (só
// add/copiar/colar/apagar), então qualquer mudança na posição dos
// keyframes sempre muda a contagem.
...
function _getSkinnedMeshByUUID(uuid) {
    if (!uuid) return null;
    return findObjectByUUID(uuid); // reaproveita o cache uuid→objeto
}
...
function applyKFData(obj, kf) {
    ...
    // Atualização do skeleton NÃO acontece aqui — quem chama applyKFData
    // (applyKeyframesAtFrame, hoje o único lugar) já faz isso em lote, uma
    // vez por skinned mesh, depois de aplicar todos os ossos. Fazer aqui
    // também, por osso, chamava skeleton.update() (que recalcula TODOS os
    // ossos do skeleton) uma vez pra cada osso individual — num rig de 50
    // ossos isso é 50x mais trabalho por frame do que o necessário, e foi
    // uma das causas do travamento ao dar play.
}
...
function setupEvents() {
    ...
    document.getElementById('animFpsInput')?.addEventListener('change', e => {
        ...
        if (v >= 1 && v <= 120) { AnimState.fps = v; markSceneDirty(); buildRuler(); renderClipsSection(); refreshAnimSidebar(); flashMessage(`FPS: ${v}`); }
...
async function _bakeImportedClip(entry) {
    ...
    try {
        ...
        markSceneDirty(); // re-flag now that the full bake (which can span many real frames) is actually done
