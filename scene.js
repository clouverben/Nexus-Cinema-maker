...
// Every shadow-casting light gets these defaults applied directly (see
// keyLight below, and _applyShadow()/addLight() for lights the person adds
// themselves) — no quality picker, no options, just one good-looking,
// mobile-safe setup. PCFSoftShadowMap (not VSM — VSM's extra blur-and-moments
// pass turned out to be too expensive together with everything else running
// and gave patchier-looking edges besides) with a moderate blur radius is
// the classic safe choice: soft, cheap, and reliable on basically anything,
// including something like a 2GB Android phone.
export const SOFT_SHADOW_DEFAULTS = { mapSize: 1024, radius: 3.5, blurSamples: 16 };
...
function makeGeometry(kind) {
  switch (kind) {
    ...
    case 'capsule':       return new THREE.CapsuleGeometry(0.32, 0.7, 8, 16);
    case 'icosahedron':   return new THREE.IcosahedronGeometry(0.58, 0);
    case 'octahedron':    return new THREE.OctahedronGeometry(0.62, 0);
    case 'tetrahedron':   return new THREE.TetrahedronGeometry(0.68, 0);
    case 'dodecahedron':  return new THREE.DodecahedronGeometry(0.58, 0);
    case 'torusKnot':     return new THREE.TorusKnotGeometry(0.34, 0.12, 64, 12);
    case 'ring':          return new THREE.RingGeometry(0.22, 0.52, 32, 2);
    case 'circle':        return new THREE.CircleGeometry(0.5, 32);
    case 'triangularPrism': return new THREE.CylinderGeometry(0.56, 0.56, 1, 3);
    case 'pyramid':       return new THREE.CylinderGeometry(0, 0.68, 1, 4);
...
function geometryHeight(kind) {
  if (kind === 'sphere' || kind === 'icosahedron' || kind === 'octahedron' ||
      kind === 'tetrahedron' || kind === 'dodecahedron') return 0.6;
  if (kind === 'torus' || kind === 'torusKnot') return 0.55;
  if (kind === 'plane' || kind === 'circle' || kind === 'ring') return 0;
  return 0.5;
}
...
let _sceneDirtyEventQueued = false;

export function markSceneDirty() {
  app.sceneDirty = true;

  // Project persistence listens to this event. Coalesce bursts of changes
  // (timeline scrubbing, particle updates, transform drags, etc.) into a
  // single notification per task so autosave can debounce without making
  // every mutation perform an IndexedDB write.
  if (_sceneDirtyEventQueued) return;
  _sceneDirtyEventQueued = true;
  queueMicrotask(() => {
    _sceneDirtyEventQueued = false;
    try { window.dispatchEvent(new CustomEvent('ncm-scene-dirty')); } catch {}
  });
}
...
export function initScene(container = document.body) {
  ...
  // NoToneMapping here is intentional, not "off": tone mapping now happens
  // exactly once, in OutputPass at the very end of the composer chain (see
  // posprocess.js ensureComposer()), instead of being baked into every
  // material's shader during the first render pass. Baking it in early used
  // to mean bloom's own HDR light got added AFTER the image was already
  // compressed to display range, so switching curves barely changed
  // anything — this is the three.js-recommended setup for postprocessing
  // + tone mapping (renderer.toneMapping = NoToneMapping, curve applied by
  // a final pass instead). setToneMappingValue()/setToneMappingExposureValue()
  // in shader.js still just set these same two properties — OutputPass
  // reads them live — so nothing about the UI wiring changes.
  ...
  // PCFSoftShadowMap: cheap, reliable, soft — see SOFT_SHADOW_DEFAULTS above
  // for why (VSM looked patchier and cost more, not worth it).
  ...
  // Mobile GPUs reclaim WebGL contexts under memory/thermal pressure or on
  // backgrounding, more aggressively than desktop. Calling preventDefault()
  // here is not optional decoration — without it the browser treats the
  // loss as permanent and never even tries to restore the context, which is
  // exactly why this used to show a stuck gray frame with no way back short
  // of quitting the app. render.js's tick() also checks app.contextLost and
  // skips rendering entirely while it's true, instead of hammering a dead
  // context every frame.
  renderer.domElement.addEventListener('webglcontextlost', (e) => {
    e.preventDefault();
    app.contextLost = true;
    console.warn('[scene] WebGL context perdido — aguardando o navegador restaurar…');
  }, false);
  renderer.domElement.addEventListener('webglcontextrestored', () => {
    app.contextLost = false;
    console.info('[scene] WebGL context restaurado — retomando o render.');
    // Three.js re-uploads textures/geometries/programs on demand as they're
    // next used, but a couple of renderer-level settings that were pushed
    // directly onto the old GL context (not tracked as "needs update" state
    // on any object) are worth re-asserting explicitly.
  ...
  // Shadows off for the scene's own default light — the shadow it was
  // casting on objects looked bad, so this stays off. Shadow config below
  // is left in place (harmless) in case shadows get turned back on for this
  // light later; lights the person adds themselves are unaffected by this
  // and still cast shadows on their own (see addLight() in lights.js).
  keyLight.castShadow = false;

  // Soft, realistic shadow out of the box — see SOFT_SHADOW_DEFAULTS above.
  // Frustum extent stays scene-specific, set here rather than in the shared
  // defaults.
  keyLight.shadow.mapSize.set(SOFT_SHADOW_DEFAULTS.mapSize, SOFT_SHADOW_DEFAULTS.mapSize);
  keyLight.shadow.radius = SOFT_SHADOW_DEFAULTS.radius;
  keyLight.shadow.blurSamples = SOFT_SHADOW_DEFAULTS.blurSamples;
...
export function addPrimitive(kind) {
  ...
  mesh.name = kind;
  mesh.userData.primitiveType = kind;
  ...
  // Plane/2D disk primitives are created lying flat, like a typical 3D DCC.
  if (kind === 'plane' || kind === 'circle' || kind === 'ring') {
    mesh.rotation.x = -Math.PI / 2;
  }
