...
// keyLight below, and _applyShadow() in main.js for lights the person adds
// themselves) — no quality picker, no options, just one good-looking,
// mobile-safe setup: VSM (Variance Shadow Map) gives soft, banding-free
// edges close to what Unity's shadows look like, for the cost of a single
// extra blur pass per light — nowhere near the cost of the GPU path tracer
// or the screen-space ray tracing pass, which resample the whole scene
// every frame. 1024px is enough resolution to look clean without being
// heavy enough to worry about on something like a 2GB Android phone.
export const SOFT_SHADOW_DEFAULTS = { mapSize: 1024, radius: 4, blurSamples: 16 };
...
export function initScene(container = document.body) {
  ...
  // VSM instead of PCFSoft: soft, banding-free shadow edges (closer to what
  // Unity's shadows look like) for the cost of one extra blur pass per
  // light — see SOFT_SHADOW_DEFAULTS above for why this is still safe on
  // something like a 2GB Android phone.
