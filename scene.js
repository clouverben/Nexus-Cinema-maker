...
// ─────────────────────────────────────────────────────────────────────────────
// SHADOW QUALITY — a small set of presets instead of exposing every raw
// three.js knob. VSM ("Alta"/"Ultra") gives genuinely soft, banding-free
// shadow edges (closer to Unity's soft shadows) for basically the cost of
// one extra blur pass per shadow-casting light — nowhere near the cost of
// the real GPU path tracer or the screen-space ray tracing pass (those build
// or sample against the WHOLE scene every frame; this only touches each
// light's own shadow map, which is what shadows have always cost here).
// "Baixa"/"Média" stay on plain PCFSoft at a small map size specifically so
// this is safe to leave on by default on something like a 2GB Android phone.
// ─────────────────────────────────────────────────────────────────────────────
export const SHADOW_QUALITY_PRESETS = {
  baixa:  { type: 'PCFSoft', mapSize: 512,  radius: 1.5, blurSamples: 8  },
  media:  { type: 'PCFSoft', mapSize: 1024, radius: 3,   blurSamples: 12 },
  alta:   { type: 'VSM',     mapSize: 1536, radius: 6,   blurSamples: 16 },
  ultra:  { type: 'VSM',     mapSize: 2048, radius: 12,  blurSamples: 24 } // Unity-like soft
};

export function applyShadowQuality(name) {
  const preset = SHADOW_QUALITY_PRESETS[name] || SHADOW_QUALITY_PRESETS.media;
  app.shadowQuality = name;

  if (app.renderer) {
    app.renderer.shadowMap.type = preset.type === 'VSM' ? THREE.VSMShadowMap : THREE.PCFSoftShadowMap;
    app.renderer.shadowMap.needsUpdate = true;
  }

  app.scene?.traverse((o) => {
    if (!o.isLight || !o.shadow) return;
    o.shadow.mapSize.set(preset.mapSize, preset.mapSize);
    o.shadow.radius = preset.radius;
    o.shadow.blurSamples = preset.blurSamples;
    // Existing shadow map render targets are baked at the OLD size — dispose
    // and null it out so three.js regenerates it at the new size/type on the
    // next shadow pass instead of silently keeping stale, wrongly-sized data.
    if (o.shadow.map) {
...
export function initScene(container = document.body) {
  ...
  // Map size/radius/blurSamples now come from applyShadowQuality() below —
  // frustum extent stays here since that's scene-specific, not a quality tier.
  ...
  // "Média" by default: soft-enough shadows (PCFSoft, 1024 map) that stay
  // safe on something like a 2GB Android phone. "Alta"/"Ultra" (VSM, bigger
  // map) are opt-in from the Avançado tab for anyone whose device can take it.
  applyShadowQuality('media');
