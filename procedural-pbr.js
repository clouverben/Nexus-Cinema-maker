...
function generateTexture(slot, cfg){
  ...
  const profile=MATERIAL_PROFILES[cfg.profile] || DEFAULT_PROFILE;
...
function applyDescriptionToState(text, targetSlot=null){
  ...
  if(resolved.isDefault){
    // "padrão" only removes the material profile. All visible numeric controls
  ...
  // The description chooses the material profile, but NEVER overwrites the
  // user's Scale/Detail/Seed/Contrast/Strength controls. Those controls are
  // the final parameters used by generateTexture().
  const slots=targetSlot && state.mapSettings[targetSlot] ? [targetSlot] : MAPS.map(([slot])=>slot);
  for(const slot of slots){
    state.mapSettings[slot].profile=resolved.name;
  }
  return {name:resolved.name, recognized:true};
}
