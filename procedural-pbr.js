...
function openPbrPrompt(targetSlot,mode='map'){
  ...
  modal.innerHTML=`
    <div class="pbrPromptHeader"><div class="pbrPromptTitle">${title}</div><button class="pbrPromptClose" type="button" aria-label="Cancelar">×</button></div>
...
function build(){
  ...
  const mapRoot=state.root;
  const labRoot=state.labRoot || document.getElementById('materialLabRoot');
  if(!mapRoot) return;

  // LEFT: only the procedural PBR controls/maps. This host is nested inside the
  // existing Material > PBR section, so the rest of the Material UI is untouched.
  mapRoot.innerHTML=`<div class="procPbrShell">
    <div class="procPbrHint">Perfil + parâmetros manuais.</div>
  ...
  if(labRoot){
    ...
    labRoot.innerHTML=`<div class="procLabViewerOnly">
      ...
      <div class="procPreviewToolbar" role="group" aria-label="Objeto do preview">
        ...
        <button type="button" class="procPreviewShape" data-preview-shape="torus">Torus</button>
      </div>
    </div>`;
  }

  const updateLab=()=>{
    const el=labRoot?.querySelector('#procLabPreview');
    labState.previewEnabled = el ? el.checked : true;
    updatePreview();
  };

  labRoot?.querySelector('#procLabPreview')?.addEventListener('input',updateLab);
  labRoot?.querySelectorAll('[data-preview-shape]').forEach(btn=>btn.addEventListener('click',()=>{
    labState.previewShape=btn.dataset.previewShape || 'sphere';
    labRoot.querySelectorAll('[data-preview-shape]').forEach(b=>b.classList.toggle('active',b===btn));
    rebuildPreviewMesh();
    updatePreview();
  }));

  mapRoot.querySelectorAll('[data-generate-proc]').forEach(btn=>btn.addEventListener('click',()=>openPbrPrompt(btn.dataset.generateProc,'map')));
  mapRoot.querySelectorAll('[data-remove-proc]').forEach(btn=>btn.addEventListener('click',()=>removeProceduralPBR()));
  MAPS.forEach(([slot])=>{
    ['scale','detail','seed','contrast','strength'].forEach(k=>{
      const el=mapRoot.querySelector(`#proc-${slot}-${k}`);
      el?.addEventListener('input',()=>{
        state.mapSettings[slot][k]=Number(el.value);
        if(k==='seed'&&labState.lockSeed){
          labState.sharedSeed=Number(el.value) || 0;
          MAPS.forEach(([s])=>state.mapSettings[s].seed=labState.sharedSeed);
          sync();
        }
        updatePreview();
      });
...
function previewGeometry(shape){
  if(shape==='cube') return new THREE.BoxGeometry(1.95,1.95,1.95,4,4,4);
  if(shape==='torus') return new THREE.TorusKnotGeometry(1.05,.36,96,24);
  return new THREE.SphereGeometry(1.22,48,32);
}
function rebuildPreviewMesh(){
  if(!previewScene) return;
  if(previewMesh){ previewScene.remove(previewMesh); previewMesh.geometry?.dispose?.(); previewMesh=null; }
  previewMesh=new THREE.Mesh(previewGeometry(labState.previewShape),new THREE.MeshPhysicalMaterial({color:0xffffff,roughness:.55,metalness:.05}));
  previewMesh.castShadow=true;
  previewScene.add(previewMesh);
  previewDirty=true;
}
function resizePreview(){
  if(!previewRenderer || !previewCamera || !state.root) return;
  const canvas=state.root.querySelector('#procPbrPreview'); if(!canvas) return;
...
function ensurePreview(){
  const canvas=state.labRoot?.querySelector('#procPbrPreview'); if(!canvas || !labState.previewEnabled) return null;
  if(previewRenderer) return canvas;
  try{
    previewRenderer=new THREE.WebGLRenderer({canvas,antialias:false,alpha:true,preserveDrawingBuffer:false,powerPreference:'low-power'});
    ...
    // Same reasoning as the main viewport's renderer (scene.js): without
    // preventDefault() here the browser never attempts to restore this
    // context, and this little preview would just go permanently blank.
    canvas.addEventListener('webglcontextlost', (e) => { e.preventDefault(); labState.previewContextLost = true; }, false);
    canvas.addEventListener('webglcontextrestored', () => { labState.previewContextLost = false; previewDirty = true; }, false);
    ...
    const fill=new THREE.PointLight(0x7da7ff,10,8,2); fill.position.set(-2.5,1.4,2.2);
    const rim=new THREE.DirectionalLight(0xffc27a,1.6); rim.position.set(-3,2,-4);
    previewFloor=new THREE.Mesh(new THREE.CircleGeometry(5,48),new THREE.MeshStandardMaterial({color:0x151922,roughness:.78,metalness:0}));
    previewFloor.rotation.x=-Math.PI/2; previewFloor.position.y=-1.22; previewFloor.receiveShadow=true;
    previewScene.add(hemi,key,fill,rim,previewFloor);
    rebuildPreviewMesh();
    previewResizeObserver=new ResizeObserver(resizePreview); previewResizeObserver.observe(canvas);
    const loop=(now)=>{
      previewFrame=requestAnimationFrame(loop);
      if(!previewMesh || !labState.previewEnabled || !previewRenderer || labState.previewContextLost) return;
      const mobile=window.matchMedia?.('(max-width: 600px)').matches;
      const interval=mobile?45:33;
      if(!previewDirty && now-previewLastRender<interval) return;
      try { previewRenderer.render(previewScene,previewCamera); }
      catch (e) { console.warn('[procedural-pbr] preview render falhou:', e); }
      previewLastRender=now; previewDirty=false;
    };
...
function updatePreview(){
  if(!state.root || !state.labRoot || !labState.previewEnabled) return;
  ensurePreview();
  if(!previewMesh) return;
  if(previewUpdateTimer) cancelAnimationFrame(previewUpdateTimer);
  previewUpdateTimer=requestAnimationFrame(()=>{
    ...
    try{
      ...
      mat.roughness=.72; mat.metalness=.08; mat.bumpScale=.08;
      mat.needsUpdate=true;
      previewDirty=true;
...
export function initProceduralPBRPanel(root, labRoot=document.getElementById('materialLabRoot')){
