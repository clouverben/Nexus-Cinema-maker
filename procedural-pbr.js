...
function build(){
  ...
  state.root.innerHTML=`<div class="procPbrShell">
    <div class="procPbrHint">Perfil + parâmetros manuais + visualização em tempo real.</div>
    <div class="procPbrLab">
      <div class="procLabHeader"><div><div class="procLabTitle">Visualizador</div><div class="procLabSub">Preview PBR otimizado para a área do painel</div></div><label class="procLabToggle"><span>Preview</span><input id="procLabPreview" type="checkbox" checked></label></div>
  ...
  state.root.querySelectorAll('[data-preview-shape]').forEach(btn=>btn.addEventListener('click',()=>{
    labState.previewShape=btn.dataset.previewShape || 'sphere';
    state.root.querySelectorAll('[data-preview-shape]').forEach(b=>b.classList.toggle('active',b===btn));
    rebuildPreviewMesh();
    updatePreview();
  }));
...
function sync(){
  ...
  state.root?.querySelectorAll('[data-preview-shape]').forEach(b=>b.classList.toggle('active',b.dataset.previewShape===labState.previewShape));
}
