...
function _ensureFinalPassTargets() {
  ...
  if (!_finalPassCompositeMat) {
    _finalPassCompositeMat = new THREE.ShaderMaterial({
      uniforms: { tBase:{value:null}, tLayer:{value:null}, uOpacity:{value:1}, uMode:{value:0} },
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          vec3 c;
          if(uMode<0.5) c=mix(b.rgb,l.rgb,a);            // normal overlay
          else if(uMode<1.5) c=b.rgb*(1.0-l.rgb*a);      // AO/shadow darken
          else c=min(vec3(1.0),b.rgb+l.rgb*a);            // additive contribution
          gl_FragColor=vec4(c,b.a);
        }`
    });
...
function _renderAovToTarget(passId, target) {
  ...
  try {
    ...
    if (passId === 'depth') {
      ...
      // AO final contributor: a screen-space darkening approximation generated from depth.
      app.renderer.overrideMaterial=_aovDepthMaterial();
      const m=_aovDepthMaterial(); m.uniforms.uNear.value=app.camera.near; m.uniforms.uFar.value=app.camera.far; m.uniforms.uRange.value=1; m.uniforms.uInvert.value=0;
    } else if (passId === 'beauty') {
      // beauty is a real rerender of the lit scene; grading is applied by the compositor weight.
    } else {
      return false;
    }
...
function _compositeFinalPassLayer(baseTarget, layerTarget, passId, settings) {
  ...
  else if (passId === 'depth' || passId === 'normal' || passId === 'objectId' || passId === 'motionVector') mode = 0;
...
function _renderFinalWithRenderPasses() {
  const selected = _finalPassDescriptors();
  if (!selected.length || !app.renderer) return false;
  if (!_ensureFinalPassTargets()) return false;

  // Render the normal NCM pipeline once into an offscreen target, then layer only
  // the passes explicitly marked "Adicionar à render final" on top of it.
  const previousTarget = app.renderer.getRenderTarget();
  app.renderer.setRenderTarget(_finalPassTargets[0]);
  app.renderer.clear(true,true,true);
  ...
  let base = composer.readBuffer || _finalPassTargets[0];

  for (const [passId, settings] of selected) {
    if (!_renderAovToTarget(passId, _finalPassLayerTarget)) continue;
    const rendered = _compositeFinalPassLayer(base, _finalPassLayerTarget, passId, settings);
    base = rendered;
  }
  _presentFinalTarget(base);
...
function _aovNormalMaterial() {
  ...
  _aovNormalMat = new THREE.MeshNormalMaterial({ side: THREE.DoubleSide });
...
export async function renderFrame() {
  ...
  if (activeRenderPass && activeRenderPass !== 'final') {
    app.renderer.overrideMaterial = null;
  ...
  // Render Pass final compositor: selected passes marked "Adicionar à render final"
