...
// ─────────────────────────────────────────────────────────────────────────────
// "LIGHT OFF" PREVIEW — every real object reads as a flat, unlit-looking clay
// surface until the light button actually starts a render. This is done as a
// render-time override material (exactly like setVisualizationMode() above),
// NOT by swapping each object's .material — swapping the reference broke the
// Material panel, since editing color there was actually editing the shared
// clay material (or losing the real one) instead of the object's own data.
// With an override, the real material is never touched, so editing it while
// the light is off works normally and simply shows once the light comes on.
// ─────────────────────────────────────────────────────────────────────────────
...
// grid is never part of this — see renderGridOverlay() below. Also the
// single source of truth for keeping every UI surface that reflects this
// state in sync (the Avançado panel's own toggle + the footer button).
export function setLightPreviewActive(active) {
  lightPreviewActive = !!active;
  setRenderModeValue(lightPreviewActive ? 'pathtracing' : 'standard');
...
// Renders the scene with every real object forced to the clay material,
// while the grid/axes are excluded from that pass entirely (hidden for it,
// then drawn back in via renderGridOverlay as a plain, un-clayed raster
// pass) so they always look normal regardless of light state.
function renderOffPreview() {
  const grid = app.gridRoot;
  const axes = app.axesHelper;
  const gridWasVisible = grid?.visible;
  const axesWasVisible = axes?.visible;
  if (grid) grid.visible = false;
  if (axes) axes.visible = false;

  app.renderer.overrideMaterial = _offPreviewMat;
  composer.render();
...
export async function renderFrame() {
  ...
  if (mode === 'standard') {
    ...
    // Light is off and nobody explicitly asked for a debug visualization
    // override — show the clay "not rendered yet" look instead of the
    // normal composite. Real materials are never modified; see
    // renderOffPreview() above.
    if (!lightPreviewActive && !app.renderer.overrideMaterial) {
      renderOffPreview();
    } else {
      composer.render();
    }
...
export function initAdvancedUI() {
    ...
    // Light starts off — markSceneDirty()'s normal render loop will pick up
    // renderOffPreview() on the very next frame since lightPreviewActive
    // defaults to false; nothing to force here.
