...
// ── Gradual post-processing fade-in ─────────────────────────────────────────
// Turning the lamp on used to switch straight from the raw render to the
// full post-processing composite in a single frame. Instead, crossfade from
// the raw render to the composited one over POST_FADE_MS, and mirror that
// same progress in the top-left "X% renderizado" overlay (see index.html).
const POST_FADE_MS = 1100;
let _postFade = { active: false, t0: 0 };
let _fadeRawRT = null;
let _fadeQuadMat = null;
let _fadeQuadScene = null;
let _fadeQuadCam = null;

function _ensureFadeResources(w, h) {
  if (!_fadeRawRT) {
    _fadeRawRT = new THREE.WebGLRenderTarget(w, h, {
      minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBAFormat,
    });
    _fadeQuadCam = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    _fadeQuadMat = new THREE.ShaderMaterial({
      uniforms: { tRaw: { value: null }, tPost: { value: null }, uMix: { value: 0 } },
      vertexShader: `varying vec2 vUv; void main(){ vUv = uv; gl_Position = vec4(position.xy, 0.0, 1.0); }`,
      fragmentShader: `
        uniform sampler2D tRaw; uniform sampler2D tPost; uniform float uMix;
        varying vec2 vUv;
        void main(){ gl_FragColor = mix(texture2D(tRaw, vUv), texture2D(tPost, vUv), uMix); }
      `,
      depthTest: false, depthWrite: false,
    });
    _fadeQuadScene = new THREE.Scene();
    _fadeQuadScene.add(new THREE.Mesh(new THREE.PlaneGeometry(2, 2), _fadeQuadMat));
  }
  if (_fadeRawRT.width !== w || _fadeRawRT.height !== h) _fadeRawRT.setSize(w, h);
}

let _renderProgressHideTimer = null;
function _setRenderProgressOverlay(pct) {
  const el = document.getElementById('renderProgressOverlay');
  if (!el) return;
  clearTimeout(_renderProgressHideTimer);
  if (pct === null) { el.classList.add('hidden'); return; }
  el.classList.remove('hidden');
  el.textContent = `${pct}% renderizado`;
  if (pct >= 100) {
    _renderProgressHideTimer = setTimeout(() => el.classList.add('hidden'), 600);
  }
}
...
export function setLightPreviewActive(active) {
  ...
  if (next && !lightPreviewActive) {
    // Rising edge: (re)start the fade-in from scratch.
    _postFade = { active: true, t0: performance.now() };
    _setRenderProgressOverlay(0);
  } else if (!next) {
    _postFade.active = false;
    _setRenderProgressOverlay(null);
  }
  lightPreviewActive = next;
...
export async function renderFrame(options = {}) {
  ...
  if (mode === 'standard') {
    ...
    if (_postFade.active) {
      const t = Math.min((performance.now() - _postFade.t0) / POST_FADE_MS, 1);
      const eased = 1 - Math.pow(1 - t, 3); // ease-out cubic — feels less mechanical than linear

      const w = app.renderer.domElement.width  || 1;
      const h = app.renderer.domElement.height || 1;
      _ensureFadeResources(w, h);

      // 1. Raw (no post-processing) render into an off-screen target.
      app.renderer.setRenderTarget(_fadeRawRT);
      app.renderer.render(app.scene, app.camera);

      // 2. Full composited render, kept off-screen (same trick used by the
      // AOV preview capture above: flip outputPass.renderToScreen off, read
      // the result back from composer.readBuffer, then restore the flag).
      const wasOnScreen = outputPass ? outputPass.renderToScreen : true;
      if (outputPass) outputPass.renderToScreen = false;
      composer.render();
      if (outputPass) outputPass.renderToScreen = wasOnScreen;

      // 3. Blend raw → composited by `eased` and present that to the canvas.
      _fadeQuadMat.uniforms.tRaw.value  = _fadeRawRT.texture;
      _fadeQuadMat.uniforms.tPost.value = composer.readBuffer.texture;
      _fadeQuadMat.uniforms.uMix.value  = eased;
      app.renderer.setRenderTarget(null);
      app.renderer.render(_fadeQuadScene, _fadeQuadCam);

      _setRenderProgressOverlay(Math.round(eased * 100));
      if (t >= 1) _postFade.active = false;
