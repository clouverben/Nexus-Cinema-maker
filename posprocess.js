...
// Push every "Path Tracer Real" panel control straight onto the live
...
function _applyRealPTControls() {
  ...
  p.filterGlossyFactor = clamp(num('adv-realpt-filterGlossy', p.filterGlossyFactor), 0, 1);
  p.tilesX             = clamp(num('adv-realpt-tilesX', p.tilesX), 1, 8);
  p.tilesY             = clamp(num('adv-realpt-tilesY', p.tilesY), 1, 8);
  p.renderScale        = clamp(num('adv-realpt-renderScale', p.renderScale), 0.1, 1);
  p.minSamples         = clamp(num('adv-realpt-minSamples', p.minSamples), 1, 128);
  p.lowResScale        = clamp(num('adv-realpt-lowResScale', p.lowResScale), 0.05, 1);
  p.dynamicLowRes       = !!document.getElementById('adv-realpt-dynamicLowRes')?.checked;
...
function _reflectRealPTControls() {
  const p = renderState.path;
  const set = (id, v) => { const el = document.getElementById(id); if (el) el.value = v; };
  set('adv-realpt-bounces',      p.bounces);
  set('adv-realpt-filterGlossy', p.filterGlossyFactor);
  set('adv-realpt-tilesX',       p.tilesX);
  set('adv-realpt-tilesY',       p.tilesY);
  set('adv-realpt-renderScale',  p.renderScale);
  set('adv-realpt-minSamples',   p.minSamples);
  set('adv-realpt-lowResScale',  p.lowResScale);
  const dlr = document.getElementById('adv-realpt-dynamicLowRes');
...
function applyTracerPreset(mode) {
  ...
  // _applyRealPTControls below), so re-creating the tracer never falls back
...
function updateSampleCounter() {
  ...
  // Mirror the same readout inline in the "Path Tracer Real" panel body.
  const progressEl = document.getElementById('adv-realpt-progress');
...
export function initAdvancedUI() {
    ...
    document.getElementById('adv-realpt-toggle')?.addEventListener('change', (e) => {
        ...
        if (on) {
            const samples = parseInt(document.getElementById('adv-realpt-samples')?.value ?? '64', 10);
    ...
    // ── Extra Path Tracer Real (GPU) controls — mirrors the shared "Samples"
    // field into the section-local one (and vice-versa) and pushes every
    // other field straight onto the live WebGLPathTracer instance so changes
    // apply immediately without a full mode toggle.
    const _rpt = (id) => document.getElementById(id);
    _rpt('adv-realpt-samples')?.addEventListener('change', (e) => {
        const v = parseInt(e.target.value, 10);
        setSamplesValuePost(Number.isFinite(v) ? v : 64);
        const shared = _rpt('adv-sampleCount');
        if (shared) shared.value = e.target.value;
    });
    ['adv-realpt-filterGlossy', 'adv-realpt-tilesX', 'adv-realpt-tilesY',
     'adv-realpt-renderScale', 'adv-realpt-minSamples', 'adv-realpt-lowResScale',
     'adv-realpt-dynamicLowRes'].forEach((id) => {
        const el = _rpt(id);
        el?.addEventListener('change', () => _applyRealPTControls());
    });
    document.getElementById('adv-realpt-reset')?.addEventListener('click', () => {
