...
function ensureComposer() {
  ...
  bloomPass  = new UnrealBloomPass(new THREE.Vector2(w, h), 0.2, 0.3, 2.0);
...
function setPipeline(mode) {
  ...
  // passthrough when bloomActive is 0, so appending it as the true final
  // pass in every branch below guarantees something always reaches the
  // screen without changing what's visually drawn.
  const tail = selectiveMixPass ? [selectiveMixPass] : [];
...
// ─────────────────────────────────────────────────────────────────────────────
// "LIGHT OFF" PREVIEW — every real object shows a flat, unlit-looking clay
// material until the light button actually starts a render. This is the
// visual cue that what's on screen right now is a draft, not the render.
// The grid/axes/floor are never touched by this (they aren't in app.objects),
// so they always read normally regardless of light state.
// ─────────────────────────────────────────────────────────────────────────────
const _offOriginalMaterials = new WeakMap();
const _offPreviewMat = new THREE.MeshStandardMaterial({
  color: 0x565b64,
  roughness: 1,
  metalness: 0,
  name: 'off-preview-clay',
});
let lightPreviewActive = false; // false = clay "off" look (default); true = light on, real render

function _forEachRealMesh(cb) {
  (app.objects || []).forEach((root) => {
    root?.traverse?.((node) => {
      if (node.isMesh && !node.userData.isHelper && !node.userData.isBoneMarker) cb(node);
    });
  });
}

function _applyOffPreview() {
  _forEachRealMesh((node) => {
    if (!_offOriginalMaterials.has(node)) _offOriginalMaterials.set(node, node.material);
    node.material = _offPreviewMat;
  });
}

function _restoreRealMaterials() {
  _forEachRealMesh((node) => {
    const orig = _offOriginalMaterials.get(node);
...
// Single entry point for the footer "light" button: ON = actually render the
// objects (GPU path tracer); OFF = back to the flat clay "off" look. The
// grid is never part of this — see renderGridOverlay() in renderFrame().
// Also the single source of truth for keeping every UI surface that reflects
// this state in sync (the Avançado panel's own toggle + the footer button).
export function setLightPreviewActive(active) {
  lightPreviewActive = !!active;
  if (lightPreviewActive) {
    _restoreRealMaterials();
  ...
  if (gptToggle) gptToggle.checked = lightPreviewActive;
  document.getElementById('gpt-body')?.classList.toggle('hidden', !lightPreviewActive);
  document.getElementById('renderLightToggleBtn')?.classList.toggle('active', lightPreviewActive);

  markSceneDirty();
}

// Called right after a new object is added to the scene so it immediately
// matches the current light state instead of showing its real material
// until the next full refresh.
export function applyLightPreviewToNewObject(root) {
  if (lightPreviewActive || !root) return;
  root.traverse?.((node) => {
    if (node.isMesh && !node.userData.isHelper && !node.userData.isBoneMarker) {
      if (!_offOriginalMaterials.has(node)) _offOriginalMaterials.set(node, node.material);
      node.material = _offPreviewMat;
    }
  });
...
// (the path-traced frame), without ever handing them to the path tracer
// itself — grid shaders/infinite-plane tricks aren't things a BVH path
// tracer can shade correctly, so it stays a plain raster overlay instead.
function renderGridOverlay() {
  ...
  app.renderer.autoClearColor = false; // keep the path-traced pixels
  app.renderer.autoClearDepth = true;  // grid has no competing depth info to test against
...
function updateSampleCounter() {
  ...
  // Mirror the same readout inline in the "Render Físico (GPU)" panel body.
  const progressEl = document.getElementById('gpt-progress');
  if (progressEl) progressEl.textContent = `${isTraced ? currentSampleCount : 0}/${target} spp`;
}
...
// ── Wiring do painel Avançado (Render Físico GPU) ───────────────────────────
export function initAdvancedUI() {
    _reflectGpuPathTracerControls();
    _applyOffPreview(); // default "off" clay look until the light is turned on
    // Registered on `app` (rather than imported into scene.js/main.js) to
    // avoid a circular module import — scene.js/main.js just call this
    // whenever a new object is added, so it immediately matches whatever
    // light state is currently active.
    app.applyLightPreview = applyLightPreviewToNewObject;
    ...
    function _reflectAdvModeButtons() {
        const on = document.getElementById('gpt-toggle')?.checked;
    ...
    advModeButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const tog = document.getElementById('gpt-toggle');
            if (!tog) return;
            const wantOn = mode === 'gpu-pt';
            if (tog.checked !== wantOn) {
                tog.checked = wantOn;
    ...
    document.getElementById('gpt-toggle')?.addEventListener('change', (e) => {
    ...
    ['gpt-filterGlossy', 'gpt-tilesX', 'gpt-tilesY', 'gpt-renderScale',
     'gpt-minSamples', 'gpt-lowResScale', 'gpt-dynamicLowRes'].forEach((id) => {
        document.getElementById(id)?.addEventListener('change', () => _applyGpuPathTracerControls());
    });
