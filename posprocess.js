...
import { buildSSRPasses, updateCubeCameraReflections } from './reflections.js';
...
export async function renderFrame(options = {}) {
  ...
  // CubeCamera reflections bake into materials, not the composer — keep
  // them live regardless of the footer lamp (post-processing on/off).
  updateCubeCameraReflections(app.renderer, app.scene);
...
// Same idea, for the SSR/CubeCamera reflections pipeline: only needed when
// an object's SSR checkbox flips or an object with it enabled is deleted —
// see reflections.js for why quality-setting tweaks don't need this.
