...
function detectPathTracerTier() {
  ...
  // Conservative mobile tiers. Unknown devices intentionally land in mid.
  if (memory <= 2 || cores <= 4 || maxTex < 4096) return 'low';
  if (memory >= 8 && cores >= 8 && maxTex >= 8192) return 'high';
  return 'mid';
}
...
function applyAdaptivePathProfile(force = false) {
  ...
  const profiles = {
    low:  { scale: 0.55, lowRes: 0.14, bounces: 2, tiles: 2, glossy: 0.55 },
    mid:  { scale: 0.72, lowRes: 0.17, bounces: 3, tiles: 2, glossy: 0.38 },
    high: { scale: 0.86, lowRes: 0.20, bounces: 4, tiles: 3, glossy: 0.25 }
  };
