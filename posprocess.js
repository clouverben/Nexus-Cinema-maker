...
// ── Progressive desktop render refinement ─────────────────────────────────
// The work viewport stays a cheap raster/PBR render. Activating the footer
// lamp starts a temporal accumulation pass inspired by SFM's Progressive
// Refinement, while keeping Prisma3D's fast mobile-friendly raster renderer
// underneath. The quality stack is therefore: PBR + scene lights + shadows +
// configured SSR/post effects + jittered TAA accumulation.
const PROGRESSIVE_RENDER_SAMPLES = 64;
let _progressiveRestartTimer = null;
let _progressiveRestartAt = 0;
let _progressiveCompleted = false;
let _progressiveForcedGtao = false;

const PROGRESSIVE_MILESTONES = [10, 20, 30, 50, 70, 100];

function _progressiveMilestone(frame, total) {
  const pct = (Math.max(0, frame) / Math.max(1, total)) * 100;
  let hit = 0;
  for (const step of PROGRESSIVE_MILESTONES) {
    if (pct >= step) hit = step;
    else break;
  }
  return hit;
}

function _syncProgressiveRenderOverlay() {
  if (!lightPreviewActive) return;
  const info = _taa.getProgress?.();
  if (!info) return;
  const hit = _progressiveMilestone(info.frame, info.maxFrames);
  _setRenderProgressOverlay(hit);
}

function _beginProgressiveQualityStack() {
  // SFM-style refinement benefits from a stable AO layer, while Prisma3D's
  // mobile-oriented raster path stays underneath for responsiveness. SSR is
  // already part of the Render panel and stays controlled there.
  _progressiveForcedGtao = !activeLayers.has('gtao');
  if (_progressiveForcedGtao) activeLayers.add('gtao');
  ensureComposer();
  setPipeline('standard');
}

function _endProgressiveQualityStack() {
  if (_progressiveForcedGtao) activeLayers.delete('gtao');
  _progressiveForcedGtao = false;
  ensureComposer();
  setPipeline('standard');
}

function _queueProgressiveRefinementRestart(delay = 180) {
  clearTimeout(_progressiveRestartTimer);
  _progressiveRestartAt = performance.now() + delay;
  _setRenderProgressOverlay(0);
}

function _cancelProgressiveRefinementRestart() {
  clearTimeout(_progressiveRestartTimer);
  _progressiveRestartTimer = null;
  _progressiveRestartAt = 0;
}
...
export function setLightPreviewActive(active) {
  ...
  if (next && !lightPreviewActive) {
    _cancelProgressiveRefinementRestart();
    _setRenderProgressOverlay(0);
    _taa.stop();
    lightPreviewActive = true;
    _progressiveCompleted = false;
    _beginProgressiveQualityStack();
    app.sceneDirty = false;
    _taa.start(PROGRESSIVE_RENDER_SAMPLES);
  } else if (!next && lightPreviewActive) {
    lightPreviewActive = false;
    _progressiveCompleted = false;
    _cancelProgressiveRefinementRestart();
    _taa.stop();
    _endProgressiveQualityStack();
    _setRenderProgressOverlay(null);
  }

  document.getElementById('renderLightToggleBtn')?.classList.toggle('active', lightPreviewActive);
  markSceneDirty();
  // The UI-state change itself is not a scene change for progressive
  // refinement. Clearing it here prevents an immediate self-restart on the
  // first frame after the lamp was switched on.
  if (lightPreviewActive) app.sceneDirty = false;
}
...
export function setRenderModeValue(mode) {
  ...
  if (mode !== 'standard') {
    if (_taa.isActive?.()) _taa.stop();
    if (lightPreviewActive) {
      _progressiveCompleted = false;
      _cancelProgressiveRefinementRestart();
      _endProgressiveQualityStack();
    }
  } else if (lightPreviewActive) {
    _progressiveCompleted = false;
    _cancelProgressiveRefinementRestart();
    _beginProgressiveQualityStack();
    _taa.start(PROGRESSIVE_RENDER_SAMPLES);
...
export async function renderFrame(options = {}) {
  ...
  // While the progressive desktop-style refinement accumulator is active,
  // it owns the frame: jitter -> render -> accumulate -> present. This is
  // deliberately separate from the GPU path tracer. The work viewport stays
  // fast, while the lamp button behaves more like SFM's progressive refinement
  // and Prisma3D's fast mobile render path: the image improves over successive
  // samples instead of switching to a heavy offline renderer.
  if (_taa.isActive()) {
    if (lightPreviewActive && renderState.mode === 'standard') {
      const moved = cameraMoved();
      const changed = !!app.sceneDirty;
      if (moved || changed) {
        app.sceneDirty = false;
        _taa.stop();
        _progressiveCompleted = false;
        _endProgressiveQualityStack();
        _queueProgressiveRefinementRestart(180);
        updateSampleCounter();
        return;
      }
    }
    ...
    if (_finalPassDescriptors().length && renderState.mode === 'standard' && !getActiveRenderPass()) {
      const accumTexture = _taa.getAccumTexture?.();
      if (accumTexture) _renderFinalWithRenderPasses(accumTexture);
    }

    if (!_taa.isActive() && lightPreviewActive) {
      _progressiveCompleted = true;
      _setRenderProgressOverlay(100);
    } else {
      _syncProgressiveRenderOverlay();
    }
  ...
  // A scene change while the progressive renderer was idle should settle for
  // a brief moment before refinement starts again. During that quiet window
  // the normal raster/composer frame remains interactive.
  if (lightPreviewActive && renderState.mode === 'standard') {
    const moved = cameraMoved();
    const changed = !!app.sceneDirty;

    if ((moved || (changed && _progressiveCompleted)) && _progressiveRestartAt === 0) {
      app.sceneDirty = false;
      _progressiveCompleted = false;
      _endProgressiveQualityStack();
      _queueProgressiveRefinementRestart(180);
    ...
    if (_progressiveRestartAt > 0 && performance.now() >= _progressiveRestartAt) {
      _progressiveRestartAt = 0;
      _beginProgressiveQualityStack();
      _taa.start(PROGRESSIVE_RENDER_SAMPLES);
      _syncProgressiveRenderOverlay();
  ...
  if (mode === 'standard') {
    ...
    // The lamp is now a render-quality switch, not a one-frame post-process
    // crossfade. Start/continue the progressive refinement accumulator. The
    // renderer remains the regular Three.js PBR pipeline (fast like Prisma3D),
    // while temporal jitter + accumulation provides SFM-style refinement.
    if (!_taa.isActive() && !_progressiveCompleted && _progressiveRestartAt === 0) {
      _beginProgressiveQualityStack();
      _taa.start(PROGRESSIVE_RENDER_SAMPLES);
    }

    // The current frame is still rendered immediately while the accumulator
    // is being scheduled, so the viewport never flashes blank.
    composer.render();
    _syncProgressiveRenderOverlay();
    updateSampleCounter();
    return;
...
export const _taa = (() => {
    ...
    return {
        ...
        isActive:  () => _running,
        getProgress: () => ({ frame: _frame, maxFrames: _maxFrames, paused: _paused, running: _running }),
        getAccumTexture: () => _accumRT?.texture || null,
    };
})();
window._taa = _taa;
