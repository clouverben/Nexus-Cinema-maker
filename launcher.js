...
let projectNameBackdrop = null;
let projectNameModal = null;
...
function showEditorShell() {
  ...
  // The editor is reused when returning from Projects. Re-apply the persisted
  // language after the editor becomes visible so rebuilt panels do not fall
  // back to Portuguese while the Settings menu still says English.
  try {
    const lang = localStorage.getItem('ncm_app_language_v1') || '';
    const apply = () => window.ncmApplyLanguage?.(lang, { silent:true });
    setTimeout(apply, 80);
    setTimeout(apply, 450);
    setTimeout(apply, 1000);
    window.dispatchEvent(new Event('ncm-editor-visible'));
  } catch (_) {}
}
...
function ensureLauncherSettings() {
  ...
  launcherSettingsMenu.innerHTML = `
    ...
    <div class="launcherSettingsBody">
      ...
      <div class="launcherSettingsSection launcherProjectSettingsSection">
        <div class="launcherSettingsSectionTitle">Projetos</div>
        <div class="launcherSettingsHint">Ações rápidas para a tela de projetos.</div>
        <div class="launcherSettingsRow launcherProjectActions">
          <button type="button" id="launcherProjectsRefresh" class="launcherSizeBtn">Atualizar projetos</button>
          <button type="button" id="launcherProjectsOpen" class="launcherSizeBtn launcherSizeBtnPrimary">Abrir editor</button>
        </div>
      </div>
      <div class="launcherSettingsSection">
  ...
  langSelect?.addEventListener('change', () => {
    ...
    // Apply to the current editor immediately when it is already mounted.
    try { window.ncmApplyLanguage?.(lang); } catch (_) {}

    // Keep the editor selector synchronized as well.
  ...
  launcherSettingsMenu.querySelector('#launcherProjectsRefresh')?.addEventListener('click', () => {
    closeLauncherSettings();
    try { window.refreshProjects?.(); } catch (_) {}
    try { renderProjectGrid?.(); } catch (_) {}
  });
  launcherSettingsMenu.querySelector('#launcherProjectsOpen')?.addEventListener('click', () => {
    closeLauncherSettings();
    try { window._openEditor?.(); } catch (_) {}
    try { ui.back?.click(); } catch (_) {}
  });
...
async function saveCurrentProject() {
  ...
  try {
    ...
    if (_saveQueued) {
      _saveQueued = false;
      // A change may have happened while IndexedDB was writing. Persist it
      // immediately after the current transaction finishes.
      queueMicrotask(() => saveCurrentProject());
    }
  }
}

function scheduleProjectAutosave(delay = 300) {
  if (!activeProjectId || launcherVisible) return;
...
function ensureProjectNameModal() {
  if (projectNameModal) return;

  projectNameBackdrop = document.createElement('div');
  projectNameBackdrop.id = 'projectNameBackdrop';
  projectNameBackdrop.className = 'projectNameBackdrop hidden';

  projectNameModal = document.createElement('div');
  projectNameModal.id = 'projectNameModal';
  projectNameModal.className = 'projectNameModal hidden';
  projectNameModal.setAttribute('role', 'dialog');
  projectNameModal.setAttribute('aria-modal', 'true');
  projectNameModal.innerHTML = `
    <div class="projectNameKicker">NEXUS ENGINE</div>
    <div class="projectNameHeader">
      <div>
        <div class="projectNameTitle">Criar novo projeto</div>
        <div class="projectNameSubtitle">Dê um nome à sua cena para começar.</div>
      </div>
      <button id="projectNameClose" class="projectNameClose" type="button" aria-label="Cancelar">×</button>
    </div>
    <label class="projectNameLabel" for="projectNameInput">Nome do projeto</label>
    <input id="projectNameInput" class="projectNameInput" type="text" maxlength="80" autocomplete="off" spellcheck="false" placeholder="Ex.: Minha animação">
    <div class="projectNameHint">Você poderá renomear o projeto depois.</div>
    <div class="projectNameActions">
      <button id="projectNameCancel" class="projectNameBtn projectNameCancel" type="button">Cancelar</button>
      <button id="projectNameCreate" class="projectNameBtn projectNameCreate" type="button">Criar projeto</button>
    </div>
  `;

  document.body.appendChild(projectNameBackdrop);
  document.body.appendChild(projectNameModal);
  projectNameInput = projectNameModal.querySelector('#projectNameInput');

  const close = () => closeProjectNameModal(true);
  projectNameBackdrop.addEventListener('click', close);
  projectNameModal.querySelector('#projectNameClose')?.addEventListener('click', close);
  projectNameModal.querySelector('#projectNameCancel')?.addEventListener('click', close);
  projectNameModal.querySelector('#projectNameCreate')?.addEventListener('click', () => confirmProjectName());
  projectNameInput?.addEventListener('keydown', e => {
    if (e.key === 'Enter') { e.preventDefault(); confirmProjectName(); }
    if (e.key === 'Escape') { e.preventDefault(); close(); }
  });
}

function closeProjectNameModal(cancel = true) {
  projectNameBackdrop?.classList.add('hidden');
  projectNameModal?.classList.add('hidden');
  if (cancel && _resolveProjectName) {
    const resolve = _resolveProjectName;
    _resolveProjectName = null;
    resolve(null);
  }
}

function openProjectNameModal() {
  ensureProjectNameModal();
  projectNameInput.value = '';
  projectNameBackdrop.classList.remove('hidden');
  projectNameModal.classList.remove('hidden');
  requestAnimationFrame(() => projectNameInput?.focus());
}

let _resolveProjectName = null;
function askProjectName() {
  openProjectNameModal();
  return new Promise(resolve => { _resolveProjectName = resolve; });
}

function confirmProjectName() {
  const clean = (projectNameInput?.value || '').trim() || 'Novo Projeto';
  closeProjectNameModal(false);
  const resolve = _resolveProjectName;
  _resolveProjectName = null;
  resolve?.(clean);
}
...
async function createNewProject() {
  const cleanName = await askProjectName();
  if (!cleanName) return;
  await saveCurrentProject();
...
function wireEvents() {
  ...
  // Safety-net autosave: much more frequent than the previous 60s interval.
  // The save itself is still debounced/coalesced, so idle projects do not
  // constantly hit IndexedDB.
  window.setInterval(() => {
